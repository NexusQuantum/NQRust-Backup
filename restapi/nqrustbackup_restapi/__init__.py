...
import nqrustbackup.util
from nqrustbackup_restapi.models import *

# Read config from api.ini
config = configparser.ConfigParser()
config_paths = []
config_override = os.environ.get("NQRUSTBACKUP_RESTAPI_CONFIG")
if config_override:
    config_paths.append(config_override)
config_paths.extend(
    [
        "api.ini",
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "api.ini"),
    ]
)
...
app = FastAPI(
    title="NQRust Backup API",
    description="NQRust Backup API for secure backup operations, job control, restore workflows, and infrastructure telemetry. **Note** Swagger does not support GET methods with bodies; the CURL statements displayed by Swagger do work.",
    version="0.0.1",
...
class UserObject(object):
    def __init__(self, username, password):
        ...
        self.director = nqrustbackup.bsock.BSock
        self.jsonDirector = nqrustbackup.bsock.DirectorConsoleJson
...
def authenticate_user(username: str, password: str):
    jsonDirector = None
    try:
        jsonDirector = nqrustbackup.bsock.DirectorConsoleJson(
            address=CONFIG_DIRECTOR_ADDRESS,
            port=CONFIG_DIRECTOR_PORT,
            dirname=CONFIG_DIRECTOR_NAME,
            name=username,
            password=nqrustbackup.bsock.Password(password),
        )
    ...
    *,
    ...
    if current_user.directorVersion > "":
        ...
        if "version" in result:
            myVersion = nqrustbackup.util.Version(result["version"]).as_python_version()
    ...
    try:
        if not (version.parse(myVersion) >= version.parse(minVersion)):
            raise HTTPException(
                status_code=501,
                detail="Not implemented in NQRust Backup %s. Need at least version %s"
...
def configure_add_standard_component(
    *,
    ...
    """
    Create a new NQRust Backup standard component resource.
...
def parseCommandOptions(queryDict):
    ...
    for q in queryDict:
        if queryDict[q] is not None:
            if type(queryDict[q]) != nqrustFlag:
...
def show_configuration_items(
    *,
    ...
    verbose: Optional[nqrustBool] = "yes",
):
    """
...
def list_catalog_items(
    *,
    ...
    verbose: Optional[nqrustBool] = "yes",
    hasCountOption: Optional[nqrustBool] = "no",
):
    itemKey = itemType
...
def read_all_clients(
    *,
    response: Response,
    current_user: User = Depends(get_current_user),
    verbose: Optional[nqrustBool] = Query("yes", title="Verbose output"),
):
    """
    Read all jobdef resources. Built on console command _show clients_.

    Needs at least NQRust Backup version >= 20.0.0
    """
...
def post_user(
    *,
    ...
    """
    Create a new NQRust Backup user resource.
...
def post_profile(
    *,
    ...
    """
    Create a new NQRust Backup profile resource.
...
def post_console(
    *,
    ...
    """
    Create a new NQRust Backup console resource.
