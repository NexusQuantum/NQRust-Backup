...
class SeleniumTest(unittest.TestCase):
    ...
    base_url = "http://127.0.0.1/nqrustbackup-webui"
    username = "admin"
    password = "secret"
    profile = "admin"
    client = "nqrust-client"
    restorefile = "/usr/sbin/nqrustbackup-console"
...
def get_env():
    ...
    chromedriverpath = os.environ.get("NQRUSTBACKUP_WEBUI_CHROMEDRIVER_PATH")
    if chromedriverpath:
        SeleniumTest.chromedriverpath = chromedriverpath

    SeleniumTest.chromeheadless = True
    chromeheadless = os.environ.get("NQRUSTBACKUP_WEBUI_CHROME_HEADLESS")
    ...
    browser = os.environ.get("NQRUSTBACKUP_WEBUI_BROWSER")
    if browser:
        SeleniumTest.browser = browser

    resolution = os.environ.get("NQRUSTBACKUP_WEBUI_BROWSER_RESOLUTION")
    if resolution:
        res = [int(i.strip()) for i in resolution.split("x", 1)]
        if len(res) != 2:
            print(
                'The variable NQRUSTBACKUP_WEBUI_BROWSER_RESOLUTION must be set like "1200 x 800"'
            )
            sys.exit(1)
        SeleniumTest.resolution_x = res[0]
        SeleniumTest.resolution_y = res[1]

    base_url = os.environ.get("NQRUSTBACKUP_WEBUI_BASE_URL")
    if base_url:
        SeleniumTest.base_url = base_url.rstrip("/")

    username = os.environ.get("NQRUSTBACKUP_WEBUI_USERNAME")
    if username:
        SeleniumTest.username = username

    password = os.environ.get("NQRUSTBACKUP_WEBUI_PASSWORD")
    if password:
        SeleniumTest.password = password

    profile = os.environ.get("NQRUSTBACKUP_WEBUI_PROFILE")
    if profile:
        SeleniumTest.profile = profile
        print("using profile:" + profile)

    testname = os.environ.get("NQRUSTBACKUP_WEBUI_TESTNAME")
    if testname:
        SeleniumTest.testname = testname

    client = os.environ.get("NQRUSTBACKUP_WEBUI_CLIENT_NAME")
    if client:
        SeleniumTest.client = client

    restorefile = os.environ.get("NQRUSTBACKUP_WEBUI_RESTOREFILE")
    if restorefile:
        SeleniumTest.restorefile = restorefile

    logpath = os.environ.get("NQRUSTBACKUP_WEBUI_LOG_PATH")
    if logpath:
        SeleniumTest.logpath = logpath

    sleeptime = os.environ.get("NQRUSTBACKUP_WEBUI_DELAY")
    if sleeptime:
