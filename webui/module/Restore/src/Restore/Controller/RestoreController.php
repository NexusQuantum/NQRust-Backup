...
class RestoreController extends AbstractActionController
{
    ...
    private function setRestoreParams()
    {
        ...
        if ($this->params()->fromQuery('mergefilesets') !== null) {
            ...
            $this->restore_params['mergefilesets'] = $_SESSION['nqrustbackup']['merge_filesets'] ? 1 : 0;
        }

        if ($this->params()->fromQuery('mergejobs') !== null) {
            $this->restore_params['mergejobs'] = $this->params()->fromQuery('mergejobs');
        } else {
            $this->restore_params['mergejobs'] = $_SESSION['nqrustbackup']['merge_jobs'] ? 1 : 0;
        }
    ...
    public function getDataAction()
    {
        ...
        if (!$this->SessionTimeoutPlugin()->isValid()) {
            return $this->redirect()->toRoute('auth', array('action' => 'login'), array('query' => array('req' => $this->RequestURIPlugin()->getRequestURI(), 'dird' => $_SESSION['nqrustbackup']['director'])));
        }
