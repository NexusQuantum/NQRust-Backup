...
class AuthController extends AbstractActionController
{
    ...
    public function loginAction()
    {
        ...
        $session = new Container('nqrustbackup');
        ...
        $product_updates = $form->getInputFilter()->getValue('product_updates');
        ...
        if (
            ...
            $this->updates = json_decode($product_updates, true);
    ...
    {
        include 'version.php'; // provides nqrustbackup_full_version (installed ui version)
        ...
        $ui_major_version = explode('.', $nqrustbackup_full_version)[0];
        if ($dird_major_version !== $ui_major_version) {
            $err_msg = 'Error: NQRust Backup WebUI (' . $nqrustbackup_full_version . ') requires a Director of the same major release (' . $dird_major_version . '). The Director version is ' . $dird_version . '.';
            return $err_msg;
        }
        return true;
    }
    ...
    {
        $err_msg_1 = 'Sorry, the user you are trying to login with has no permissions for the .api command. Contact your NQRust administrator for NQRust Backup console profile support.';
        $err_msg_2 = 'Error: This NQRust Backup Director does not support the required API 2 mode.';
    ...
    {
        $err_msg = 'Sorry, your Command ACL does not fit the minimal requirements. Contact your NQRust administrator for NQRust Backup profile support.';
    ...
    /**
     * get update info for NQRustBackup Director.
     *
    ...
    private function getUpdateInfoDIRD()
    {
        ...
        if (isset($dird_version)) {
            return \NQRustBackup\Util::getNearestVersionInfo($this->updates, $dird_version);
        }
