...
Since :sinceVersion:`15.2.0: nqrustbackup-webui` |webui| is part of the NQRustBackup project and available for a number of platforms.

.. image:: /include/images/nqrustbackup-webui-jobs.*
   :width: 80.0%
...
-  Individual accounts and ACL support via NQRustBackup restricted named consoles
...
-  nqrustbackup_console interface (limited to non-interactive commands)

System Requirements
-------------------

-  A platform for which the **nqrustbackup-webui** package is available, see :ref:`section-NQRustBackupPackages`

-  A working NQRustBackup environment
...
Adding the NQRustBackup Repository
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If not already done, add the NQRustBackup repository that is matching your Linux distribution. Please have a look at the chapter :ref:`section-InstallNQRustBackupPackages` for more information on how to achieve this.

Install the nqrustbackup-webui package
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

After adding the repository simply install the nqrustbackup-webui package via your package manager.

-  RHEL, CentOS and Fedora

   .. code-block:: shell-session

      yum install nqrustbackup-webui

   or

   .. code-block:: shell-session

      dnf install nqrustbackup-webui

-  SUSE Linux Enterprise Server (SLES), openSUSE

   .. code-block:: shell-session

      zypper install nqrustbackup-webui

-  Debian, Ubuntu

   .. code-block:: shell-session

      apt-get install nqrustbackup-webui
...
#. Use :command:`nqrustbackup_console` to create a user with name **admin** and password **secret** and permissions defined in :config:option:`Dir/Profile = webui-admin`\ :
   ...
   Or rename :file:`/etc/nqrustbackup/nqrustbackup-dir.d/console/admin.conf.example` to `.conf` and reload |dir| configuration.

#. Login to http://HOSTNAME/nqrustbackup-webui with username and password as created.
...
Upgrade from NQRustBackup 21 or lower to NQRustBackup 22
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Therefore, the |webui| Director profile :config:option:`dir/profile = webui-admin` has been extended by the directive :config:option:`dir/profile/PluginOptionsACL = *all*` in NQRustBackup :sinceVersion:`22.0.0: PluginOptionsACL = *all* added to profile webui-admin`.
...
-  create a file :file:`/etc/nqrustbackup/nqrustbackup-dir.d/console/admin.conf` with following content:

   .. code-block:: nqrustbackupconfig
      :caption: nqrustbackup-dir.d/console/admin.conf
...
-  or use the :command:`nqrustbackup_console`:
   ...
   TLS-PSK is not available between the |webui| and the |dir|. To enable TLS with certificates, see :ref:`TransportEncryptionWebuiNQRustBackupDirChapter`.
...
The package **nqrustbackup-webui** comes with a predefined profile for |webui|: :config:option:`Dir/Profile = webui-admin`\ .

If your |webui| is installed on another system than the |dir|, you have to copy the profile to the |dir|.

This is the default profile, giving access to all NQRustBackup resources and allowing all commands used by the |webui|:

.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/profile/webui-admin.conf
...
.. index::
   single: SELinux; nqrustbackup-webui
...
.. index::
   single: AppArmor; nqrustbackup-webui
...
.. code-block:: shell-session
   ...
   # nqrustbackup-webui
   /usr/share/nqrustbackup-webui/** r,
   /etc/nqrustbackup-webui/directors.ini r,
   /etc/nqrustbackup-webui/configuration.ini r,
...
.. index::
   single: Transport Encryption; nqrustbackup-webui

See the dedicated chapter and section :ref:`TransportEncryptionWebuiNQRustBackupDirChapter`\.
...
.. index::
   single: Apache; nqrustbackup-webui

The package **nqrustbackup-webui** provides a default configuration for Apache. Depending on your distribution, it is installed at :file:`/etc/apache2/conf.d/nqrustbackup-webui.conf`, :file:`/etc/httpd/conf.d/nqrustbackup-webui.conf`, :file:`/etc/apache2/available-conf/nqrustbackup-webui.conf` or similar.

The required Apache modules, :strong:`fcgid` and :strong:`rewrite` are enabled via package postinstall script. However, after installing the **nqrustbackup-webui** package, you need to restart your Apache webserver manually.


.. _section-webui-configuration-files:

Configure your /etc/nqrustbackup-webui/directors.ini
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. index::
   single: Configuration; WebUI

Configure your directors in :file:`/etc/nqrustbackup-webui/directors.ini` to match your settings.

The configuration file :file:`/etc/nqrustbackup-webui/directors.ini` should look similar to this:

.. code-block:: nqrustbackupconfig
   :caption: /etc/nqrustbackup-webui/directors.ini

   ;
   ; NQRustBackup WebUI Configuration File
   ;
   ; File: /etc/nqrustbackup-webui/directors.ini
   ;
   ...
   ; E.g. ca_file = "/etc/nqrustbackup-webui/tls/NQRustBackupCA.crt"
   ;ca_file = ""

   ; Path to the cert file which needs to contain the client certificate and the key in PEM encoding
   ; E.g. ca_file = "/etc/nqrustbackup-webui/tls/restricted-named-console.pem"
   ;cert_file = ""
...
Configure your :file:`/etc/nqrustbackup-webui/configuration.ini`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Some parameters of the |webui| can be configured in :file:`/etc/nqrustbackup-webui/configuration.ini`:

.. code-block:: nqrustbackupconfig
   :caption: /etc/nqrustbackup-webui/configuration.ini

   ;
   ; NQRustBackup WebUI Configuration File
   ;
   ; File: /etc/nqrustbackup-webui/configuration.ini
   ;
...
.. code-block:: nqrustbackupconfig
   ...
   Job {
     ...
     JobDefs = "DefaultJob"
     WriteBootstrap = "|/usr/sbin/nqrustbackup_smtp -h localhost -f "(NQRustBackup) " -s "Bootstrap for Job %j" root@localhost"
     Priority = 100
     run before job = "/usr/lib/nqrustbackup/scripts/make_catalog_backup MyCatalog"
     run after job = "/usr/lib/nqrustbackup/scripts/delete_catalog_backup MyCatalog"
     Run Script {
       Console = ".bvfs_update"
       RunsWhen = After
       RunsOnClient = No
...
.. index::
   single: nginx; nqrustbackup-webui

If you prefer to use |webui| on Nginx with php-fpm instead of Apache, a basic working configuration could look like this:

.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-webui on nginx

   server {

           listen       9100;
           server_name  nqrustbackup;
           root         /var/www/nqrustbackup-webui/public;
...
This will make the |webui| accessible at http://nqrustbackup:9100/ (assuming your DNS resolve the hostname :strong:`nqrustbackup` to the NGINX server).
...
A complete overview of :command:`nqrustbackup_console` command usage in the |webui| can be found in
...
the jobs :strong:`backup-nqrustbackup-fd` and :strong:`backup-example-fd`.
...
.. code-block:: cfg
   ...
   Profile {
      ...
      JobACL = backup-nqrustbackup-fd, backup-example-fd, RestoreFiles
      ScheduleACL = WeeklyCycle
      CatalogACL = MyCatalog
      PoolACL = Full, Differential, Incremental
      StorageACL = File
      ClientACL = nqrustbackup-fd, example-fd
...
.. code-block:: cfg
   ...
   Profile {
     ...
     Job ACL = backup-nqrustbackup-fd, RestoreFiles
...
.. image:: /include/images/nqrustbackup-webui-restore-0.*
   :width: 80%
...
Restore location on client
   ...
   In case you want to use another location, simply enter the path here where you want to restore to on the selected client, for example :file:`/tmp/nqrustbackup-restore/`.
...
.. image:: /include/images/nqrustbackup-webui-restore-1.*
   :width: 80%

Restore a specific file version
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. image:: /include/images/nqrustbackup-webui-restore-2.*
   :width: 80%
...
.. note::
   ...
   Restoring NDMP backups is currently not supported by NQRustBackup WebUI.
   Please use the nqrustbackup_console instead.
...
If you have (or want) a service contract for NQRustBackup, the Backup Unit Report can
help you keep track of the subscriptions in use.
It will compute how many NQRustBackup backup units are consumed by your installation,
...
Once logged into your NQRustBackup WebUI, navigate to **Director -> Subscription**.
...
.. image:: /include/images/webui-status-subscription-report.png
   :alt: NQRustBackup WebUI: Subscription Status Overview
   :width: 100.0%

| Alternatively you can download the report directly via the following URL:
| ``https://<YOUR_NQRUSTBACKUP_WEBUI_SERVER>/nqrustbackup-webui/director/backupunitreport``
| Or for the anonymized report:
| ``https://<YOUR_NQRUSTBACKUP_WEBUI_SERVER>/nqrustbackup-webui/director/backupunitreport?anonymize=1``
