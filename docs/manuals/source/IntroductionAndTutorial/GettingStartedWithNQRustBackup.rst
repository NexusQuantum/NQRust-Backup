...
Getting Started with NQRustBackup
===========================
...
If you have been using a program such as :command:`tar` to backup your system, Pools, Volumes, and labeling may be a bit confusing at first. A Volume is a single physical tape (or possibly a single file) on which NQRustBackup will write your backup data. Pools group together Volumes so that a backup is not restricted to the length of a single Volume (tape). Consequently, rather than explicitly naming Volumes in your Job, you specify a Pool, and NQRustBackup will select the next appendable Volume
from the Pool and mounts it.

Although the basic Pool options are specified in the Director’s :ref:`Pool <DirectorResourcePool>` resource, the real Pool is maintained in the NQRustBackup Catalog. It contains information taken from the Pool resource (configuration file) as well as information on all the Volumes that have been added to the Pool.

For each Volume, NQRustBackup maintains a fair amount of catalog information such as the first write date/time, the last write date/time, the number of files on the Volume, the number of bytes on the Volume, the number of Mounts, etc.

Before NQRustBackup will read or write a Volume, the physical Volume must have a NQRustBackup software label so that NQRustBackup can be sure the correct Volume is mounted. Depending on your configuration, this is either done automatically by NQRustBackup or manually using the :bcommand:`label` command in the Console program.
...
the daily and weekly backup Jobs, you thereby insure that no daily Job ever writes to a Volume in the Weekly Pool and vice versa, and NQRustBackup will tell you what tape is needed and when.
...
Setting Up NQRustBackup Configuration Files
-------------------------------------

:index:`\ <single: Configuration; Files>`\

On Unix, NQRustBackup configuration files are usually located in the :file:`/etc/nqrustbackup/` directory and are named accordingly to the programs that use it. Since NQRustBackup :sinceVersion:`16.2.4: Subdirectory Configuration Scheme used as Default` the default configuration is stored as one file per resource in subdirectories under :file:`nqrustbackup-dir.d`, :file:`nqrustbackup-sd.d` or :file:`nqrustbackup-fd.d`. For details, see
...
As the |dir| and |sd| runs as user **nqrustbackup**, testing the configuration should be done as **nqrustbackup**.

This is especially required to test the |dir|, as it also connects to the database and checks if the catalog schema version is correct. Depending on your database, only the **nqrustbackup** has permission to access it.

.. code-block:: shell-session
   :caption: Testing Configuration Files

   su nqrustbackup -s /bin/sh -c "/usr/sbin/nqrustbackup-dir -t"
   su nqrustbackup -s /bin/sh -c "/usr/sbin/nqrustbackup-sd -t"
   nqrustbackup-fd -t
   nqrustbackup_console -t
   nqrustbackup-tray-monitor -t
