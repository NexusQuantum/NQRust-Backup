NDMP Backups with NQRustBackup
========================
...
The NQRustBackup NDMP implementation is based on the NDMJOB NDMP reference implementation of Traakan, Inc., Los Altos, CA which has a BSD style license (2 clause one) with some enhancements.
...
NDMP Backup in NQRustBackup
---------------------

NQRustBackup offers two types of NDMP integration:
...
When using NDMP_NQRUSTBACKUP, the |sd| acts as Tape Agent.
...
.. csv-table::
   :header: "", |ndmpnqrustbackup|, |ndmpnative|
...
.. _section-NdmpNQRustBackup:

NDMP_NQRUSTBACKUP
-----------

NQRustBackup implements the Data Management Agent inside of the |dir| and a Tape Agent in the |sd|.

The Tape Agent in the |sd| emulates a NDMP tape drive that has an infinite tape. Because of the infinite tape, no Robot Agent is required and therefore not implemented. The blocks being written to the NDMP tape are wrapped into a normal NQRustBackup backup stream and then stored into the volumes managed by NQRustBackup.

There is always a pair of storage resource definitions:

-  a conventional NQRustBackup storage resource and

-  a NDMP storage resource

These two are linked together. Data that is received by the Tape Agent inside of the |sd| is then stored as NQRustBackup backup stream inside of the paired conventional NQRustBackup storage resource.

On restore, the data is read by the conventional resource, and then recovered as NDMP stream from the NDMP resource.

.. note:: Copying and migrating a NDMP_NQRUSTBACKUP job is not an NDMP operation. NDMP jobs are copied and migrated just like every other NQRustBackup job.


.. figure:: /include/images/ndmp-backup.*
   :alt: Relationship between NQRustBackup and NDMP components

   Relationship between NQRustBackup and NDMP components

Example Setup for NDMP_NQRUSTBACKUP backup
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

:index:`\ <single: NDMP; Example; NDMP\_NQRUSTBACKUP>`\

This example starts from a clean default NQRustBackup installation.
...
NQRustBackup Director: Configure NDMP Client Resource
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Verify, that you can access your Primary Storage System via NQRustBackup:
...
NQRustBackup Storage Daemon: Configure NDMP
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. code-block:: nqrustbackupconfig
   ...
   #
   # This resource gives the DMA in the Director access to the NQRustBackup SD via the NDMP protocol.
   # This option is used via the NDMP protocol to open the right TAPE AGENT connection to your
   # NQRustBackup SD via the NDMP protocol. The initialization of the SD is done via the native protocol
   # and is handled via the PairedStorage keyword.
   #
   Ndmp {
     Name = nqrustbackup-dir-isilon
     Username = ndmpadmin
     Password = test
     AuthType = Clear
   }
...
.. code-block:: shell-session
   ...
   root@host:~# netstat -lntp | grep nqrustbackup-sd
   tcp        0      0 0.0.0.0:9103            0.0.0.0:*               LISTEN      10661/nqrustbackup-sd
   tcp        0      0 0.0.0.0:10000           0.0.0.0:*               LISTEN      10661/nqrustbackup-sd


NQRustBackup Director: Configure a Paired Storage
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. code-block:: nqrustbackupconfig
   ...
   # We link via the PairedStorage config option the NQRustBackup SD
...
.. code-block:: nqrustbackup_console
   ...
   *<input>status storage=NDMPFile</input>
   Connecting to Storage daemon File at nqrustbackup:9103

   nqrustbackup-sd Version: 15.2.2 (16 November 2015) x86_64-redhat-linux-gnu redhat Red Hat Enterprise Linux Server release 7.0 (Maipo)
   ...
   Device "FileStorage" (/var/lib/nqrustbackup/storage) is not open.
   ==
   ====
...
NQRustBackup Director: Configure NDMP Fileset
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. warning::
   ...
   Normally (:config:option:`dir/client/Protocol`\ =Native) Filesets get handled by the \nqrustbackupFd. When connecting directly to a NDMP Clients (:config:option:`dir/client/Protocol`\ =NDMP*), no |fd| is involved and therefore most Fileset options can't be used. Instead, parameters are handled via :strong:`Options - Meta`\  from :config:option:`dir/fileset/Include`\ .


.. warning::

   Avoid using multiple :config:option:`dir/fileset/Include`\  :strong:`File`\  directives.
   The |dir| would try to handle them by running multiple NDMP jobs in a single NQRustBackup job.
...
HIST
   ...
   N
      Disables file history. Without file history, single file restore is not possible with NQRustBackup.

   Some NDMP environments (eg. Isilon OneFS) allow additional parameter:

   F
      Specifies path-based file history. This is the most efficient with NQRustBackup.
...
NQRustBackup Director: Configure NDMP Jobs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. code-block:: nqrustbackup_console
   ...
   *<input>list joblog jobid=1</input>
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Start NDMP Backup JobId 1, Job=NDMPJob.2016-01-14_10.57.51_04
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Created new Volume "Full-0001" in catalog.
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Using Device "FileStorage" to write.
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Opening tape drive LPDA-DEJC-ENJL-AHAI-JCBD-LICP-LKHL-IEDK@/ifs/home%0 read/write
    2016-01-14 10:57:53 nqrustbackup-sd JobId 1: Labeled new Volume "Full-0001" on device "FileStorage" (/var/lib/nqrustbackup/storage).
    2016-01-14 10:57:53 nqrustbackup-sd JobId 1: Labeled new Volume "Full-0001" on device "FileStorage" (/var/lib/nqrustbackup/storage)
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Commanding tape drive to rewind
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Waiting for operation to start
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Async request NDMP4_LOG_MESSAGE
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Operation started
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: Monitoring backup
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: LOG_MESSAGE: 'Filetransfer: Transferred 5632 bytes in 0.087 seconds throughput of 63.133 KB/s'
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: LOG_MESSAGE: 'Filetransfer: Transferred 5632 total bytes '
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: LOG_MESSAGE: 'CPU  user=0.016416  sys=0.029437  ft=0.077296  cdb=0.000000'
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: LOG_MESSAGE: 'maxrss=14576  in=13  out=22  vol=155  inv=72'
    2016-01-14 10:57:53 nqrustbackup-dir JobId 1: LOG_MESSAGE: '
    ...
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: Async request NDMP4_NOTIFY_MOVER_HALTED
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: DATA: bytes 2053KB  MOVER: written 2079KB record 33
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: Operation done, cleaning up
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: Waiting for operation to halt
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: Commanding tape drive to NDMP9_MTIO_EOF 2 times
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: Commanding tape drive to rewind
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: Closing tape drive LPDA-DEJC-ENJL-AHAI-JCBD-LICP-LKHL-IEDK@/ifs/home%0
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: Operation halted, stopping
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: Operation ended OKAY
    2016-01-14 10:57:54 nqrustbackup-sd JobId 1: Elapsed time=00:00:01, Transfer rate=2.128 M Bytes/second
    2016-01-14 10:57:54 nqrustbackup-dir JobId 1: NQRustBackup nqrustbackup-dir 15.2.2 (16Nov15):
...
.. code-block:: nqrustbackup_console
   ...
   *<input>restore jobid=1</input>
   ...
   $ <input>done</input>
   Bootstrap records written to /var/lib/nqrustbackup/nqrustbackup-dir.restore.1.bsr
   ...
   Run Restore job
   JobName:         ndmp-backup-job
   Bootstrap:       /var/lib/nqrustbackup/nqrustbackup-dir.restore.1.bsr
   Where:           /
   Replace:         Always
   FileSet:         NDMP Fileset
   ...
   *<input>list joblog jobid=2</input>
   14-Jan 11:04 nqrustbackup-dir JobId 2: Start Restore Job ndmp-backup-job.2016-01-14_11.04.53_05
   14-Jan 11:04 nqrustbackup-dir JobId 2: Using Device "FileStorage" to read.
   14-Jan 11:04 nqrustbackup-dir JobId 2: Opening tape drive KKAE-IMLO-NHJD-GOCO-GJCO-GEHB-BODL-ADNG@/ifs/home read-only
   14-Jan 11:04 nqrustbackup-dir JobId 2: Commanding tape drive to rewind
   14-Jan 11:04 nqrustbackup-dir JobId 2: Waiting for operation to start
   14-Jan 11:04 nqrustbackup-sd JobId 2: Ready to read from volume "Full-0001" on device "FileStorage" (/var/lib/nqrustbackup/storage).
   14-Jan 11:04 nqrustbackup-sd JobId 2: Forward spacing Volume "Full-0001" to file:block 0:194.
   14-Jan 11:04 nqrustbackup-dir JobId 2: Async request NDMP4_LOG_MESSAGE
   14-Jan 11:04 nqrustbackup-dir JobId 2: Operation started
   14-Jan 11:04 nqrustbackup-dir JobId 2: Monitoring recover
   14-Jan 11:04 nqrustbackup-dir JobId 2: DATA: bytes 0KB  MOVER: read 0KB record 0
   14-Jan 11:04 nqrustbackup-dir JobId 2: LOG_MESSAGE: 'Filetransfer: Transferred 1048576 bytes in 0.135 seconds throughput of 7557.139 KB/s'
   14-Jan 11:04 nqrustbackup-dir JobId 2: OK: /admin/.zshrc
   14-Jan 11:04 nqrustbackup-dir JobId 2: LOG_MESSAGE: '
           Objects:
           ----------------------------
           Regular Files:          (1)
           Stub Files:             (0)
   ...
   14-Jan 11:04 nqrustbackup-dir JobId 2: LOG_MESSAGE: '
           ...
           Max Size:               643
           Mean Size:              643'
   14-Jan 11:04 nqrustbackup-dir JobId 2: Async request NDMP4_NOTIFY_MOVER_PAUSED
   14-Jan 11:04 nqrustbackup-dir JobId 2: DATA: bytes 1024KB  MOVER: read 2079KB record 32
   14-Jan 11:04 nqrustbackup-dir JobId 2: Mover paused, reason=NDMP9_MOVER_PAUSE_EOF
   14-Jan 11:04 nqrustbackup-dir JobId 2: End of tapes
   14-Jan 11:04 nqrustbackup-dir JobId 2: DATA: bytes 1024KB  MOVER: read 2079KB record 32
   14-Jan 11:04 nqrustbackup-dir JobId 2: Operation done, cleaning up
   14-Jan 11:04 nqrustbackup-dir JobId 2: Waiting for operation to halt
   14-Jan 11:04 nqrustbackup-dir JobId 2: Commanding tape drive to rewind
   14-Jan 11:04 nqrustbackup-dir JobId 2: Closing tape drive KKAE-IMLO-NHJD-GOCO-GJCO-GEHB-BODL-ADNG@/ifs/home
   14-Jan 11:04 nqrustbackup-dir JobId 2: Operation halted, stopping
   14-Jan 11:04 nqrustbackup-dir JobId 2: Operation ended OKAY
   14-Jan 11:04 nqrustbackup-dir JobId 2: LOG_FILE messages: 1 OK, 0 ERROR, total 1 of 1
   14-Jan 11:04 nqrustbackup-dir JobId 2: NQRustBackup nqrustbackup-dir 15.2.2 (16Nov15):
     Build OS:               x86_64-redhat-linux-gnu redhat Red Hat Enterprise Linux Server release 7.0 (Maipo)
     JobId:                  2
...
=============================== ===================================== ===============================================
:file:`/ifs/home/admin/.zshrc`  :file:`/nqrustbackup-restores`               :file:`/ifs/home/nqrustbackup-restores/admin/.zshrc`
:file:`/ifs/home/admin/.zshrc`  :file:`^/ifs/data/nqrustbackup-restores`     :file:`/ifs/data/nqrustbackup-restores/admin/.zshrc`
=============================== ===================================== ===============================================
...
We set up an additional |sd| on a host named :strong:`nqrustbackup-sd2.example.com` with the default :config:option:`Sd/Storage = FileStorage`\  device.

When this is done, add a second storage resource :config:option:`Dir/Storage = File2`\  to the :file:`nqrustbackup-dir.conf`:
...
.. code-block:: nqrustbackupconfig
   ...
   Storage {
     Name = File2
     Address = nqrustbackup-sd2.example.com
...
.. code-block:: nqrustbackupconfig
   ...
   Pool {
     Name = Copy
     Pool Type = Backup
     Recycle = yes                       # NQRustBackup can automatically recycle Volumes
...
.. code-block:: nqrustbackup_console
   ...
   *<input>list joblog jobid=74</input>
   21-Jan 09:19 nqrustbackup-dir JobId 74: The following 1 JobId was chosen to be copied: 73
   21-Jan 09:19 nqrustbackup-dir JobId 74: Automatically selected Catalog: MyCatalog
   21-Jan 09:19 nqrustbackup-dir JobId 74: Using Catalog "MyCatalog"
   21-Jan 09:19 nqrustbackup-dir JobId 75: Copying using JobId=73 Job=NDMPJob.2016-01-21_09.18.50_49
   21-Jan 09:19 nqrustbackup-dir JobId 75: Bootstrap records written to /var/lib/nqrustbackup/nqrustbackup-dir.restore.20.bsr
   21-Jan 09:19 nqrustbackup-dir JobId 74: Job queued. JobId=75
   21-Jan 09:19 nqrustbackup-dir JobId 74: Copying JobId 75 started.
   21-Jan 09:19 nqrustbackup-dir JobId 74: NQRustBackup nqrustbackup-dir 15.2.2 (16Nov15):
     Build OS:               x86_64-redhat-linux-gnu redhat Red Hat Enterprise Linux Server release 7.0 (Maipo)
     Current JobId:          74
   ...
   21-Jan 09:19 nqrustbackup-dir JobId 75: Start Copying JobId 75, Job=NDMPCopy.2016-01-21_09.19.52_51
   21-Jan 09:19 nqrustbackup-dir JobId 75: Using Device "FileStorage" to read.
   21-Jan 09:19 nqrustbackup-dir JobId 76: Using Device "FileStorage2" to write.
   21-Jan 09:19 nqrustbackup-sd JobId 75: Ready to read from volume "Full-0001" on device "FileStorage" (/var/lib/nqrustbackup/storage).
   21-Jan 09:19 nqrustbackup-sd JobId 76: Volume "Copy-0004" previously written, moving to end of data.
   21-Jan 09:19 nqrustbackup-sd JobId 76: Ready to append to end of Volume "Copy-0004" size=78177310
   21-Jan 09:19 nqrustbackup-sd JobId 75: Forward spacing Volume "Full-0001" to file:block 0:78177310.
   21-Jan 09:19 nqrustbackup-sd JobId 75: End of Volume at file 0 on device "FileStorage" (/var/lib/nqrustbackup/storage), Volume "Full-0001"
   21-Jan 09:19 nqrustbackup-sd JobId 75: End of all volumes.
   21-Jan 09:19 nqrustbackup-sd JobId 76: Elapsed time=00:00:01, Transfer rate=64.61 K Bytes/second
   21-Jan 09:19 nqrustbackup-dir JobId 75: NQRustBackup nqrustbackup-dir 15.2.2 (16Nov15):
     Build OS:               x86_64-redhat-linux-gnu redhat Red Hat Enterprise Linux Server release 7.0 (Maipo)
...
.. code-block:: nqrustbackupmessage

   21-Jan 09:21 nqrustbackup-dir JobId 77: Fatal error: Read storage File2 doesn't point to storage definition with paired storage option.
...
Also we have to configure NDMP on the |sd| :strong:`nqrustbackup-sd2.example.com`. For this follow the instruction from :ref:`section-ndmp-sd-configure`.

After this, a restore from :strong:`nqrustbackup-sd2.example.com` directly to the NDMP Primary Storage System is possible.

Limitations
~~~~~~~~~~~

   This list the specific limitations of the NDMP_NQRUSTBACKUP protocol. For limitation for all NQRustBackup NDMP implementation, see :ref:`section-NdmpCommonLimitations`.
...
.. limitation:: NDMP: File information are not available in the NQRustBackup backup stream.

   As hard link information is only stored in the NQRustBackup database, but not int the backup stream itself, it is not possible to recover the file history information from the NDMP stream with :command:`nqrustbackup_scan`.
...
Contrary to |ndmpnative|, the |ndmpnqrustbackup| implementation do not support NDMP "Direct Access Restore" (DAR).
...
The reason for this is that the Primary Storage System handles the backup data by itself. NQRustBackup will not modify the backup data it receives from the Primary Storage System.
...
The NDMP_NATIVE protocol is implemented since NQRustBackup :sinceVersion:`17.2.3: NDMP NATIVE`.

NQRustBackup implements the Data Management Agent inside of the |dir| and is the only NQRustBackup Daemon involved in the backups.
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/Client/isilon.conf
...
Verify, that you can access your Primary Storage System (Tape Agent) via NQRustBackup:
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/Fileset/isilon.conf
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/Storage/isilon.conf
...
.. code-block:: nqrustbackup_console
   ...
   *<input>status storage=isilon</input>
   Tape Agent isilon.nqrustbackup.com NDMPv4
     Host info
       hostname   isilon
       os_type    Isilon OneFS
       os_vers    v7.2.1.4
   ...
   Robot Agent isilon.nqrustbackup.com NDMPv4
     Host info
       hostname   isilon
       os_type    Isilon OneFS
       os_vers    v7.2.1.4
...
We now know the device names and can configure what robot and what tape to use when this storage is used by nqrustbackup by updating the :config:option:`Sd/Storage = isilon`\  resource:
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/Pool/NDMP-Tape.conf
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/Job/ndmp-native-backup-job.conf
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/Job/ndmp-native-restore-job.conf
...
.. code-block:: nqrustbackup_console
   ...
   *<input>run job=ndmp-native-backup-job yes</input>
   ...
   JobId 1: Using Data  host isilon.nqrustbackup.com
   JobId 1: Using Tape  host:device  isilon.nqrustbackup.com:HP-TLD-004-01
   JobId 1: Using Robot host:device  isilon.nqrustbackup.com:mc001
   ...
   JobId 1: NQRustBackup nqrustbackup-dir 17.2.3:
...
.. code-block:: nqrustbackup_console
   ...
   *<input>restore</input>
   ...
   cwd is: /
   : mark /ifs/home/testdata/git/nqrustbackup/src/console/nqrustbackup_console
   1 file marked.
   : mark /ifs/home/testdatrandom5G-2
   1 file marked.
   $ done
   Connecting to Director nqrustbackup:9101
   1000 OK: nqrustbackup-dir Version: 17.2.3
   ...
   JobId 2: Namelist add: node:6033077461, info:40076288, name:"/ifs/home/testdata/git/nqrustbackup/src/console/nqrustbackup_console"
   ...
   JobId 2: OK: /testdata/git/nqrustbackup/src/console/nqrustbackup_console
   ...
   JobId 2:          slot empty=N bad=N missing=N
   JobId 2: NQRustBackup nqrustbackup-dir 17.2.3:
     Build OS:               x86_64-unknown-linux-gnu redhat Red Hat Enterprise Linux Server release 6.8 (Santiago)
     JobId:                  2
...
This section contains additional information about the NQRustBackup NDMP implementation that are valid for all NQRustBackup NDMP protocols.
...
.. code-block:: nqrustbackupmessage
   ...
   24-Feb 08:40 nqrustbackup-dir JobId 14: Error: LOG_MESSAGE: 'Invalid backup level "10".'
   24-Feb 08:40 nqrustbackup-dir JobId 14:  ERR NDMP4_DATA_START_BACKUP  NDMP4_ILLEGAL_ARGS_ERR
...
NQRustBackup NDMP Common Limitations
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. limitation:: NDMP: No single file restore on merged backups.
   ...
   Unfortunately, it is currently (nqrustbackup-15.2.2) not possible to restore a chain of Full and Incremental backups at once.
...
NQRustBackup NDMP support have been tested against:

.. csv-table::
   :header: Vendor, Product, "NDMP Subsystem", "NQRustBackup version", "Tape Agent", Features, Remarks

   Isilon,     Isilon OneFS v7.2.1.4, Isilon NDMP 2.2.1   , nqrustbackup-17.2.3, Isilon Backup Accelerator, ,             Protocol: |ndmpnative|
   Isilon,     Isilon OneFS v7.2.0.1, Isilon NDMP 2.2     , nqrustbackup-16.2.6, |sd|
   Isilon,     Isilon OneFS v7.1.1.5, Isilon NDMP 2.2     , nqrustbackup-15.2.2, |sd|
   NetApp,                          , Release 8.2.3 7-Mode, nqrustbackup-15.2.2, |sd|
   Oracle/Sun, ZFS Storage Appliance, OS 8.3              , nqrustbackup-15.2.2, |sd|
