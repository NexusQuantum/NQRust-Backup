...
-  The clients (nqrustbackup-fd) must run as root to be able to access all the system files.
...
-  You should restrict access to the NQRustBackup configuration files, so that the passwords are not world-readable. The NQRustBackup daemons are password protected using CRAM-MD5 (i.e. the password is not sent across the network). This will ensure that not everyone can access the daemons. It is a reasonably good protection, but can be cracked by experts.
...
-  You should ensure that the NQRustBackup working directories are readable and writable only by the NQRustBackup daemons.

-  Don’t forget that NQRustBackup is a network program, so anyone anywhere on the network with the console program and the Director’s password can access NQRustBackup and the backed up data.

-  You can restrict what IP addresses NQRustBackup will bind to by using the appropriate Address records in the respective daemon configuration files.
...
NQRustBackup supports the secure erase of files that usually are simply deleted. NQRustBackup uses an external command to do the secure erase itself.
...
.. code-block:: nqrustbackuplog
   :caption: nqrustbackup.log

   08-Sep 12:58 win-fd JobId 10: secure_erase: executing C:/cygwin64/bin/shred.exe "C:/temp/nqrustbackup-restores/C/Program Files/NQRustBackup/Plugins/nqrustbackup_fd_consts.py"
   08-Sep 12:58 win-fd JobId 10: secure_erase: executing C:/cygwin64/bin/shred.exe "C:/temp/nqrustbackup-restores/C/Program Files/NQRustBackup/Plugins/nqrustbackup_sd_consts.py"
   08-Sep 12:58 win-fd JobId 10: secure_erase: executing C:/cygwin64/bin/shred.exe "C:/temp/nqrustbackup-restores/C/Program Files/NQRustBackup/Plugins/bpipe-fd.dll"
...
To run NQRustBackup on an OS that is running in `FIPS` mode, some adjustment need to be made so that NQRustBackup only uses algorithms and protocols that are available in the `FIPS` mode.
...
The above test procedure should work out of the box, and NQRustBackup daemons should select only FIPS approved protocols.
...
As all MD5 related function are disabled in `FIPS` mode, NQRustBackup emits errors
...
With these modifications, NQRustBackup can be run on a FIPS enabled Operating System.
