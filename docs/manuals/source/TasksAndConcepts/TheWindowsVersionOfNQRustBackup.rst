...
The Windows Version of NQRustBackup
=============================

.. index:: Platform; Windows
.. index:: Windows

The Windows version of NQRustBackup is a native Win32 port, but there are very few source code changes to the Unix code, which means that the Windows version is for the most part running code that has long proved stable on Unix systems.

Chapter :ref:`SupportedOSes` shows, what Windows versions are supported.

The NQRustBackup component that is most often used in Windows is the File daemon or Client program. As a consequence, when we speak of the Windows version of NQRustBackup below, we are mostly referring to the File daemon (client).

Once installed NQRustBackup normally runs as a system service. This means that it is immediately started by the operating system when the system is booted, and runs in the background even if there is no user logged into the system.
...
Normally, you will install the Windows version of NQRustBackup from the binaries.
The Windows version of NQRustBackup **winnqrustbackup** binary packages are provided
in the :file:`windows` directory of the NQRustBackup release directory.
The current NQRustBackup Community Repository is at https://download.nqrustbackup.org/current/windows/
while the NQRustBackup Subscription Repository of NQRustBackup release 25 is at
https://download.nqrustbackup.com/nqrustbackup/release/25/windows/.

This install is standard Windows .exe that runs an install wizard using the NSIS Free Software installer, so if you have already installed Windows software, it should be very familiar to you. Providing you do not already have NQRustBackup installed, the installer installs the binaries and dlls in :file:`C:\\Program Files\\NQRustBackup` and the configuration files in :file:`C:\\ProgramData\\NQRustBackup` (for Windows XP and older:
:file:`C:\\Documents and Settings\\All Users\\Application Data\\NQRustBackup`).

In addition, the Start\->All Programs\->NQRustBackup menu item will be created during the installation, and on that menu, you will find items for editing the configuration files, displaying the document, and starting a user interface.

During installation you can decide, what NQRustBackup components you want to install.

Typically, you only want to install the NQRustBackup Client (|fd|) and optionally some interface tools on a Windows system. Normally, we recommend to let the server components run on a Linux or other Unix system. However, it is possible, to run the |dir|, |sd| and |webui| on a Windows systems. You should be aware about following limitations:
...
.. limitation:: Windows: The default installation of |webui| is only suitable for local access.
   ...
   the configuration shipped the the **winnqrustbackup** package uses the PHP internal webserver.
...
-  You need to fill in the name of your nqrustbackup director in the client configuration dialogue and the FQDN or ip address of your client.
...
-  Add the client resource to your NQRustBackup Director Configuration and a job resource for the client as it is also described in the default nqrustbackup-dir.conf
...
/DIRECTORADDRESS
   sets network address of the director for nqrustbackup_console access

/DIRECTORNAME
   sets the name of the director to access the client and of the director to accessed by nqrustbackup_console
...
/INSTALLDIRECTOR
   install the NQRustBackup Director (and nqrustbackup_console). :sinceVersion:`14.2.1: Windows Installation: INSTALLDIRECTOR`

/INSTALLSTORAGE
   install the NQRustBackup Storage Daemon. :sinceVersion:`14.2.1: Windows Installation: INSTALLSTORAGE`
...
By setting the Installation Parameters via commandline and using the silent installer, you can install the nqrustbackup client without having to do any configuration after the installation e.g. as follows:



::

   c:\winnqrustbackup.exe /S /CLIENTNAME=hostname-fd /CLIENTPASSWORD="verysecretpassword" /DIRECTORNAME=nqrustbackup-dir



DBADMINUSER and DBADMINPASSWORD are used to create the nqrustbackup databases. If login is not possible silent installer will abort
...
VSS is available since Windows XP. From the perspective of a backup-solution for Windows, this is an extremely important step. VSS allows NQRustBackup to backup open files and even to interact with applications like RDBMS to produce consistent file copies. VSS aware applications are called VSS Writers, they register with the OS so that when NQRustBackup wants to do a Snapshot, the OS will notify the register Writer programs, which may then create a consistent state in their application, which will be backed
...
NQRustBackup produces a message from each of the registered writer programs when it is doing a VSS backup so you know which ones are correctly backed up.

Technically NQRustBackup creates a shadow copy as soon as the backup process starts. It does then backup all files from the shadow copy and destroys the shadow copy after the backup process. Please have in mind, that VSS creates a snapshot and thus backs up the system at the state it had when starting the backup. It will disregard file changes which occur during the backup process.
...
::
   ...
   nqrustbackup-fd Version: 14.2.6 (07 Dec 2015) VSS Windows XP MVS NT 5.1.2600
...
::
   ...
   23-Jul 13:25 nqrustbackup-dir: Start Backup JobId 1, Job=NightlySave.2015-07-23_13.25.45
   23-Jul 13:26 nqrustbackup-sd: Labeled new Volume "TestVolume001" on device "DDS-4" (/dev/nst0)
   23-Jul 13:26 nqrustbackup-sd: Spooling data ...
   23-Jul 13:26 nqrustbackup-fd: Generate VSS snapshots. Driver="VSS WinXP", Drive(s)="C"
   23-Jul 13:26 nqrustbackup-fd: VSS Writer: "MSDEWriter", State: 1 (VSS_WS_STABLE)
   23-Jul 13:26 nqrustbackup-fd: VSS Writer: "Microsoft Writer (Bootable State)", State: 1 (VSS_WS_STABLE)
   23-Jul 13:26 nqrustbackup-fd: VSS Writer: "WMI Writer", State: 1 (VSS_WS_STABLE)
   23-Jul 13:26 nqrustbackup-fd: VSS Writer: "Microsoft Writer (Service State)", State: 1 (VSS_WS_STABLE)
...
If you are not using the :ref:`VSS` option and if any applications are running during the backup and they have files opened exclusively, NQRustBackup will not be able to backup those files, so be sure you close your applications (or tell your users to close their applications) before the backup. Fortunately, most Microsoft applications do not open files exclusively so that they can be backed up. However, you will need to experiment. In any case, if NQRustBackup cannot open the file, it will
print an error message, so you will always know which files were not backed up. If Volume Shadow Copy Service is enabled, NQRustBackup is able to backing up any file.
...
Besides normal files and directories, Windows filesystems also support special files, called "Reparse Points". NQRustBackup can handle the following types of Reparse points:
...
.. code-block:: shell-session
   ...
   08/07/2014  03:05 PM    <DIR>          ..
   08/07/2014  02:59 PM    <SYMLINKD>     dirlink [C:\Program Files\NQRustBackup]
   08/07/2014  03:02 PM    <SYMLINK>      filelink [C:\Program Files\NQRustBackup\nqrustbackup-dir.exe]
   08/07/2014  03:00 PM    <JUNCTION>     junction [C:\Program Files\NQRustBackup]
...
Symbolic Links. Directory Symbolic Links, and Junctions that are not a Volume MountPoint are treated by NQRustBackup as symbolic links and are backed up and restored as they are, so the object is restored and points to where it pointed when it was backed up.
...
NQRustBackup automatically converts these entries to wildcards which will be automatically excluded from backup.
...
More details can be found if the filedaemon is run in debug mode inside of the :file:`nqrustbackup-fd.trace` logfile. Each entry and the resulting wildcard are logged.

.. code-block:: nqrustbackupmessage
   :caption: translation between registry key FilesNotToBackup and NQRustBackup Exclude FileSet
...
The Windows builtin Firewall is enabled since Windows version WinXP SP2. The NQRustBackup installer opens the required network ports for NQRustBackup. However, if you are using another Firewall, you might need to manually open the NQRustBackup network ports. The |fd| listens on 9102/TCP.
...
Therefore it is advised to add the :command:`nqrustbackup-fd.exe` to the list of excluded monitored processes on computers being backed up via the nqrustbackup filedaemon.
On systems running the director or storage daemon on windows, also add the processes
:command:`nqrustbackup-dir.exe`, :command:`nqrustbackup-sd.exe`
...
If you are not using the portable option, and you have :config:option:`dir/fileset/EnableVss`\  (Volume Shadow Copy) enabled in the |dir| and you experience problems with NQRustBackup not being able to open files, it is most likely that you are running an antivirus program that blocks NQRustBackup from doing certain operations. In this case, disable the antivirus program and try another backup. If it succeeds, either get a different (better) antivirus program or use
something like :config:option:`dir/job/ClientRunBeforeJob`\ /:config:option:`dir/job/ClientRunAfterJob`\  to turn off the antivirus program while the backup is running.
...
In case of problems, you can enable the creation of log files. For this you have to use the :command:`nqrustbackup_console` :ref:`setdebug <bcommandSetdebug>` command:

.. code-block:: nqrustbackup_console
   :caption: Enable debug

   *<input>setdebug client=nqrustbackup-fd level=200 trace=1</input>
   Connecting to Client nqrustbackup-fd at nqrustbackup.example.com:9102
   2000 OK setdebug=200 trace=1 hangup=0 tracefile=c:\nqrustbackup-fd.trace
...
.. code-block:: powershell
   :caption: Extract the last 1'000 lines of log from NQRustBackup Application to a csv file in powershell

   Get-EventLog -LogName Application -Source NQRustBackup | Select -First 1000 | Export-Csv "C:\temp\nqrustbackup.csv" -NoTypeInformation -UseCulture
...
In general, specifying these two options should allow you to backup any file on a Windows system. However, in some cases, if users have allowed to have full control of their folders, even system programs such a NQRustBackup can be locked out. In this case, you must identify which folders or files are creating the problem and do the following:
...
::
      ...
      Keywords:      Classic
      User:          N/A
      Computer:      nqrustbackup-windows-fd
      Description:
      ...
      <System>
         ...
         <Computer>nqrustbackup-windows-fd</Computer>
         <Security />
      </System>
      <EventData>
...
If you are experiencing problems such as VSS hanging on MSDE, first try running vssadmin to check for problems, then try running Windows Backup Feature which also uses VSS to see if it has similar problems. If so, you know that the problem is in your Windows machine and not with NQRustBackup.
...
If you restore files backed up from Windows to an alternate directory, NQRustBackup may need to create some higher level directories that were not saved (or restored). In this case, the File daemon will create them under the SYSTEM account because that is the account that NQRustBackup runs under as a service and with full access permission. However, there may be cases where you have problems accessing those files even if you run as administrator. In principle, Microsoft supplies you with the way to cease
...
If you have not installed NQRustBackup while running as Administrator and if NQRustBackup is not running as a Process with the userid (User Name) SYSTEM, then it is very unlikely that it will have sufficient permission to access all your files.

Some users have experienced problems restoring files that participate in the Active Directory. They also report that changing the userid under which NQRustBackup (nqrustbackup-fd.exe) runs, from SYSTEM to a Domain Admin userid, resolves the problem.
...
-
   ...
   ``HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\NQRustBackup-fd``

You can use the command :command:`regedit` to modify the settings.

E.g. to always start NQRustBackup in debug mode, modify ``HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\NQRustBackup-fd`` :file:`ImagePath` from



::

   "C:\Program Files\NQRustBackup\nqrustbackup-fd.exe" /service
...
::
   ...
   "C:\Program Files\NQRustBackup\nqrustbackup-fd.exe" /service -d200

After restarting the service, you will find a file called :file:`C:\nqrustbackup-fd.trace` which will contain the debug output created by the daemon.
...
To create two nqrustbackup-fd services, you can call the following service create calls on the commandline on windows as administrator:

.. code-block:: shell-session

   sc create nqrustbackupfd2 binpath="\"C:\Program Files\NQRustBackup\nqrustbackup-fd.exe\" /service  -c \"C:\ProgramData\NQRustBackup2\""  depend= "tcpip/afd"

This will create a second |fd| service with the name nqrustbackupfd2.

The configuration directories for the two services is :file:`\"C:\\ProgramData\\NQRustBackup2\\\"` needs to have different network settings.

The service can be started by calling

.. code-block:: shell-session

   sc start nqrustbackupfd2
...
These options are not normally seen or used by the user, and are documented here only for information purposes. At the current time, to change the default options, you must either manually run NQRustBackup or you must manually edit the system registry and modify the appropriate entries.

In order to avoid option clashes between the options necessary for NQRustBackup to run on Windows and the standard NQRustBackup options, all Windows specific options are signaled with a forward slash character (/), while as usual, the standard NQRustBackup options are signaled with a minus (-), or a minus minus (``--``). All the standard NQRustBackup options can be used on the Windows version. In addition, the following Windows only options are implemented:

/service
   Start NQRustBackup as a service

/run
   Run the NQRustBackup application

/install
   Install NQRustBackup as a service in the system registry

/remove
   Uninstall NQRustBackup from the system registry

/about
   Show the NQRustBackup about dialogue box

/status
   Show the NQRustBackup status dialogue box

/events
   Show the NQRustBackup events dialogue box (not yet implemented)

/kill
   Stop any running NQRustBackup

/help
   Show the NQRustBackup help dialogue box

It is important to note that under normal circumstances the user should never need to use these options as they are normally handled by the system automatically once NQRustBackup is installed. However, you may note these options in some of the .bat files that have been created for your use.
