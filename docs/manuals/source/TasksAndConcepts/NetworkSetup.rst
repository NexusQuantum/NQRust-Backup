...
In NQRustBackup the *Heartbeat Interval* is an optional parameter expressed in seconds and
...
The NQRustBackup default value is zero (0), which means no change is made to the socket.
...
.. code-block:: nqrustbackupconfig
   :caption: Enable passive mode in nqrustbackup-dir.d/client/myself.conf
...
NQRustBackup supports network topologies where the |fd| and |sd| are situated inside of a LAN,
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/client/FD\_LAN.conf
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/client/SD\_LAN.conf
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/client/FD\_WAN.conf
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/client/SD\_WAN.conf
...
To circumvent these problems, since NQRustBackup :sinceVersion:`16.2.2: Client Initiated Connection` it is possible to let the |fd| initiate the network connection to the |dir|.
...
``Append = "/var/log/nqrustbackup/nqrustbackup-fd.log" = all, !skipped, !restored``

to the default message resource :config:option:`Fd/Messages = Standard`\ :

.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-fd.d/messages/Standard.conf

   Messages {
     Name = Standard
     Director = nqrustbackup-dir = all, !skipped, !restored
     Append = "/var/log/nqrustbackup/nqrustbackup-fd.log" = all, !skipped, !restored
   }
...
The following diagrams show NQRustBackup components with any possible
...
------------------------
This diagram contains all possible connections between NQRustBackup components
that are virtually usable. See the chapters :ref:`below for specific diagrams <ConnecionOverviewNamedConsoleAndDefaultConsole>` of the NQRustBackup operating modes.
