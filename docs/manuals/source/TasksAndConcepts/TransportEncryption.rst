...
NQRustBackup uses TLS (Transport Layer Security) to provide secure network transport. For data encryption in contrast, please see the :ref:`DataEncryption` chapter. The initial Bacula encryption implementation has been written by Landon Fuller.
...
:config:option:`dir/director/TlsEnable`\
   Enable TLS support. This is by default enabled. If no certificates are configured PSK (Pre Shared Keys) ciphers will be used. If the other side does not support TLS, or cleartext is configured the connection will be aborted. However, for downward compatibility with clients before NQRustBackup-18.2 the daemons can omit transport encryption and cleartext will be sent.

:config:option:`dir/director/TlsRequire`\
   Require TLS connection, for downward compatibility. This is by default disabled. However, if :strong:`TlsRequire`\ =yes, clients with a version before NQRustBackup-18.2 will be denied if configured to use cleartext.
...
NQRustBackup is known to work well with RSA certificates.
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/director/nqrustbackup-dir.conf

   Director {                            # define myself
       Name = nqrustbackup-dir
       ...
       TLS Enable = yes     #yes by default
       TLS CA Certificate File = /etc/nqrustbackup/tls/ca.pem
       # This is a server certificate, used for incoming
       # (console) connections.
       TLS Certificate = /etc/nqrustbackup/tls/nqrustbackup-dir.example.com-cert.pem
       TLS Key = /etc/nqrustbackup/tls/nqrustbackup-dir.example.com-key.pem
       TLS Verify Peer = yes
       TLS Allowed CN = "nqrustbackup@backup1.example.com"
       TLS Allowed CN = "administrator@example.com"
   }

.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/storage/File.conf

   Storage {
       Name = File
       Address = nqrustbackup-sd1.example.com
       ...
       TLS Enable = yes     #yes by default
       TLS CA Certificate File = /etc/nqrustbackup/tls/ca.pem
       ...
       TLS Certificate = /etc/nqrustbackup/tls/nqrustbackup-dir.example.com-cert.pem
       TLS Key = /etc/nqrustbackup/tls/nqrustbackup-dir.example.com-key.pem
       TLS Allowed CN = nqrustbackup-sd1.example.com
   }

.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-dir.d/client/client1-fd.conf
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-sd.d/storage/nqrustbackup-sd1.conf

   Storage {
       Name = nqrustbackup-sd1
       ...
       ...
       TLS Certificate = /etc/nqrustbackup/tls/nqrustbackup-sd1.example.com-cert.pem
       TLS Key = /etc/nqrustbackup/tls/nqrustbackup-sd1.example.com-key.pem
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-sd.d/director/nqrustbackup-dir.conf
   ...
   Director {
       ...
       TLS Verify Peer = yes
       TLS Allowed CN = "nqrustbackup-dir.example.com"
   }
...
.. code-block:: nqrustbackupconfig
   :caption: nqrustbackup-fd.d/client/myself.conf
   ...
   Client {
       ...
       TLS CA Certificate File = /etc/nqrustbackup/tls/ca.pem
       TLS Certificate = /etc/nqrustbackup/tls/client1.example.com-cert.pem
       TLS Key = /etc/nqrustbackup/tls/client1.example.com-key.pem
...
.. code-block:: nqrustbackupconfig
   ...
   Director {
       ...
       TLS Certificate = /etc/nqrustbackup/tls/client11.example.com-cert.pem
       TLS Key = /etc/nqrustbackup/tls/client1.example.com-key.pem
       TLS Verify Peer = yes
...
Compatibility with |nqrustbackupFD|
-----------------------------

|nqrustbackupFD| connection handshake probing
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

As from NQRustBackup 18.2 all components by default establish a secure connection with encryption first, followed by the proprietary NQRustBackup protocol. This is accomplished using TLS-PSK. Older components of NQRustBackup than version 18.2 start a connection with a cleartext handshake without encryption.

For downward compatibility NQRustBackup Director Daemons and NQRustBackup Storage Daemons are able to connect to NQRustBackup File Daemons older than version 18.2. In this case Director and Storage switch to the old protocol.

There are two connection modes of a File Daemon, active and passive. In contrast to a connection from an active NQRustBackup File Daemon, the protocol version of a passive File Daemon has to be probed by the Director Daemon initially when a job is initiated. This information is stored in the configuration and immediately submitted to the Storage Daemon when the job is started.

The following sequence is used to figure out the right protocol version and to submit this information to the attached NQRustBackup Storage Daemon:

.. uml::
  :caption: Sequence diagram of a NQRustBackup File Daemon connection
...
|nqrustbackupFD| 18.2 with NQRustBackup before 18.2
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

|nqrustbackupFD| >= 18.2 can be used on a NQRustBackup system before NQRustBackup-18.2.

The *older* |nqrustbackupDir| and |nqrustbackupSD| connect to |nqrustbackupFD| using the cleartext NQRustBackup handshake before they can switch to TLS. If you want transport encryption then only TLS with certificates can be used. TLS-PSK is not possible with |nqrustbackupDir| and |nqrustbackupSd| before NQRustBackup-18.2.
...
|nqrustbackupDir| configuration
^^^^^^^^^^^^^^^^^^^^^^^^^

.. code-block:: ini
  :caption: :file:`/etc/nqrustbackup/nqrustbackup-dir.d/client/nqrustbackup-fd.conf`
...
.. code-block:: ini
  :caption: :file:`/etc/nqrustbackup/nqrustbackup-dir.d/storage/nqrustbackup-sd.conf`
...
|nqrustbackupSD| configuration
^^^^^^^^^^^^^^^^^^^^^^^^

.. code-block:: ini
  :caption: :file:`/etc/nqrustbackup/nqrustbackup-sd.d/storage/nqrustbackup-sd.conf`
...
|nqrustbackupFD| configuration before 18.2
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. code-block:: ini
  :caption: :file:`/etc/nqrustbackup/nqrustbackup-fd.d/client/nqrustbackup-fd.conf`
...
.. code-block:: ini
  :caption: :file:`/etc/nqrustbackup/nqrustbackup-fd.d/director/nqrustbackup-dir.conf`
...
|nqrustbackupFD| before 18.2 with NQRustBackup 18.2
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

|nqrustbackupFD| *before* 18.2 can be used on a NQRustBackup system 18.2 *onwards*.

The newer |nqrustbackupDir| and |nqrustbackupSD| connect to |nqrustbackupFD| using the cleartext NQRustBackup handshake before they switch to TLS. If you want transport encryption only TLS with certificates can be used, not PSK as it is possible with NQRustBackup 18.2.

However, it is also possible to disable transport encryption and use cleartext transport using the following configuration changes:

|nqrustbackupFD| configuration
^^^^^^^^^^^^^^^^^^^^^^^^
...
.. _TransportEncryptionWebuiNQRustBackupDirChapter:

|nqrustbackupWebui|
-------------

Transport encryption between |nqrustbackupWebui| and a |nqrustbackupDir| can be configured on a per restricted named console basis.

TLS-PSK is not available between the NQRustBackup WebUI and the NQRustBackup Director, in the following you will set up TLS with certificates.
...
.. note::
   ...
   For |nqrustbackupWebui| the certificate file given by configuration parameter cert_file in directors.ini has to contain the certificate and the key in PEM encoding.
...
.. warning::
   ...
   In NQRustBackup version 18.2, the global certificates configured in the director resource in the director configuration need to be used.
   Before, the certificates configured in the console resource of the director configuration were used.


.. code-block:: ini
   :caption: :file:`/etc/nqrustbackup-webui/directors.ini`
   ...
   ca_file = "/etc/nqrustbackup-webui/tls/ca.crt"
   cert_file = "/etc/nqrustbackup-webui/tls/client.pem"
   ;cert_file_passphrase = ""
   ;allowed_cns = ""

.. code-block:: ini
   :caption: :file:`/etc/nqrustbackup/nqrustbackup-dir.d/director/nqrustbackup-dir.conf`

   Director {
      Name = nqrustbackup-dir
      QueryFile = "/usr/lib/nqrustbackup/scripts/query.sql"
      ...
      # Additionally the Heartbeat can be enabled in nqrustbackup-sd and nqrustbackup-fd.
      #
      ...
      TLS Require = no
      TLS Verify Peer = no
      TLS CA Certificate File = /etc/nqrustbackup/tls/ca.crt
      TLS Certificate = /etc/nqrustbackup/tls/server.crt
      TLS Key = /etc/nqrustbackup/tls/server.pem
   }

.. _directors-ini-directives:

Overview of the settings in the |nqrustbackupWebui| :file:`directors.ini` file
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. csv-table:: TLS settings for |nqrustbackupWebui|
   :header-rows: 1
...
To be able to communicate via TLS, TLS needs to be configured on both sides. In NQRustBackup certain directives are used to set up TLS.

The following table explains the location of the relevant TLS configuration directives for all possible NQRustBackup TCP connections. Each resource is referred to as <component>-<resource> to identify the exact configuration location. Refer to chapter :ref:`ConfigureChapter` for more details about configuration.

In NQRustBackup Version 18.2 the relevant resources for some connections had to be changed. Affected directives are marked with the applicable version and the respective resource is written in bold letters.

*Remark: TLS-PSK is not available on NQRustBackup components before Version 18.2.*

 .. csv-table:: TLS Configuration Reference
    :file: nqrustbackup_connection_modes_overview_1.csv
    :widths: 20 35 10 35
...
.. note::
   ...
   NQRustBackup cannot influence this decision directly.

To make it more likely that |ktls| is used, you may consult your operating system vendors manual to
see which TLS ciphers it supports and configure nqrustbackup to use those.

.. warning::
   Some SSL implementations do not correctly report whether |ktls| is used or not.
   In that case NQRustBackup may send data completely unencrypted!  Make sure to study
   the respective manuals.

If `EnableKtls` was set to **Yes** then NQRustBackup will emit a debug message saying whether
it thinks that |ktls| is enabled or not.

.. note::
   On some operating systems you have to prepare |ktls| before nqrustbackup can take
...
.. code-block:: shell-session
   ...
   host:~$ nqrustbackup_console -d150
   [...]
   nqrustbackup_console (100): lib/tls_openssl_private.cc:58-0 Construct TlsOpenSslPrivate
   nqrustbackup_console (100): lib/tls_openssl_private.cc:614-0 Set tcp filedescriptor: <3>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:632-0 Set protocol:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:554-0 Set ca_certfile:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:560-0 Set ca_certdir:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:566-0 Set crlfile_:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:572-0 Set certfile_:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:578-0 Set keyfile_:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:596-0 Set dhfile_:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:620-0 Set cipherlist:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:626-0 Set ciphersuites:	<>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:602-0 Set Verify Peer:	<false>
   nqrustbackup_console (100): lib/tls_openssl_private.cc:608-0 Set ktls:	<true>
   nqrustbackup_console (50): lib/tls_openssl.cc:63-0 Preparing TLS_PSK CLIENT context for identity R_CONSOLE *UserAgent*
   nqrustbackup_console (100): lib/tls_openssl_private.cc:541-0 psk_client_cb. identity: R_CONSOLE *UserAgent*.
   nqrustbackup_console (150): lib/tls_openssl_private.cc:436-0 Ktls used for Recv: no
   nqrustbackup_console (150): lib/tls_openssl_private.cc:437-0 Ktls used for Send: yes
   [...]
...
With TLS/PSK activated by default in NQRustBackup since version 18, it is sometimes mandatory to achieve better performance and increase the throughput of backups and restores.
...
To do that, we only need to change two parameters into nqrustbackup configuration files, but their syntax and the location are a bit complex.
...
Just be sure they are present on all hosts you want to use with NQRustBackup.
...
Currently only SHA256 ciphers are supported by nqrustbackup.
...
.. code-block:: cfg
   :caption: List of configuration files where to apply changes
    ...
    nqrustbackup-dir.d/director/nqrustbackup-dir.conf
    nqrustbackup-dir.d/client/nqrustbackup-fd.conf
    nqrustbackup-dir.d/storage/File.conf

    nqrustbackup-fd.d/client/myself.conf
    nqrustbackup-fd.d/director/nqrustbackup-dir.conf

    nqrustbackup-sd.d/storage/nqrustbackup-sd.conf
    nqrustbackup-sd.d/director/nqrustbackup-dir.conf

    nqrustbackup_console.conf
...
.. code-block:: shell
   :caption: Example of default ciphers for nqrustbackup-fd before restricted configuration
...
.. code-block:: shell
   :caption: Example of TLS restricted protocol and ciphers for nqrustbackup-fd only TLS-PSK
...
With the restricted configuration in place, the signature in :command:`nqrustbackup_console` will reflect the changes.
...
.. code-block:: nqrustbackup_console
   ...
   nqrustbackup_console
    ...
    1000 OK: nqrustbackup-dir Version: 21.1.5 (09 November 2022)
