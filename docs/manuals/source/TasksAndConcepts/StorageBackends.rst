...
A NQRustBackup Storage Daemon can use various storage backends:

**Tape**
   is used to access tape device and thus has sequential access.

**File**
   tells NQRustBackup that the device is a file. It may either be a file defined on fixed medium or a removable filesystem such as USB. All files must be random access devices.
...
A GlusterFS Storage can be used as Storage backend of NQRustBackup. Prerequisites are a working
GlusterFS storage system and the package **nqrustbackup-storage-glusterfs**.
See https://www.gluster.org/ for more information regarding GlusterFS installation and
configuration and specifically `https://docs.gluster.org/en/latest/Administrator-Guide/NQRustBackup/ <https://docs.gluster.org/en/latest/Administrator-Guide/NQRustBackup/>`__
for NQRustBackup integration. You can use following snippet to configure it as storage device:
...
A dedupable device splits up the data it receives into nqrustbackup specific data and file data
