...
NQRustBackup permits file data encryption and signing within the File Daemon (or Client) prior to sending data to the Storage Daemon. Upon restoration, file signatures are validated and any mismatches are reported. At no time does the Director or the Storage Daemon have access to unencrypted file contents.
...
Example Data Encryption Configurations (nqrustbackup-fd.conf)
-------------------------------------------------------
...
-  Set the PKI Keypair statement in your nqrustbackup configuration file:
