...
The rest of this chapter will explain how to setup NQRustBackup so that it would automatically manage a set of disk files with the minimum sysadmin intervention.
...
One needs to consider about what happens if we have only a single large NQRustBackup Volume defined on our hard disk. Everything works fine until the Volume fills, then NQRustBackup will ask you to mount a new Volume. This same problem applies to the use of tape Volumes if your tape fills. Being a hard disk and the only one you have, this will be a bit of a problem. It should be obvious that it is better to use a number of smaller Volumes and arrange for NQRustBackup to automatically recycle them so that the disk
...
.. code-block:: nqrustbackupconfig
   ...
   JobDefs {
     ...
     Priority = 10
     Write Bootstrap = "/var/lib/nqrustbackup/%c.bsr"
   }
   ...
   Messages {
     Name = Standard
     mailcommand = "nqrustbackup_smtp -h mail.domain.com -f \"\(NQRustBackup\) %r\"
         -s \"NQRustBackup: %t %e of %c %l\" %r"
     operatorcommand = "nqrustbackup_smtp -h mail.domain.com -f \"\(NQRustBackup\) %r\"
         -s \"NQRustBackup: Intervention needed for %j\" %r"
     mail = root@domain.com = all, !skipped
     operator = root@domain.com = mount
     console = all, !skipped, !saved
     append = "/home/nqrustbackup/bin/log" = all, !skipped
   }
...
.. code-block:: nqrustbackupconfig
   ...
   Device {
     ...
     LabelMedia = yes;    # lets NQRustBackup label unlabeled media
     Random Access = yes;
