...
Of all the configuration files needed to run NQRustBackup, the Director’s is the most complicated and the one that you will need to modify the most often as you add clients or modify the FileSets.
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-director-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-director-description.rst.inc
...
.. warning::
  ...
  NQRustBackup uses only :config:option:`dir/job/Client` and :config:option:`dir/job/FileSet` to determine which jobids belong together.
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-job-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-job-description.rst.inc
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-schedule-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-schedule-description.rst.inc

Note, the Week of Year specification wnn follows the ISO standard definition of the week of the year, where Week 1 is the week in which the first Thursday of the year occurs, or alternatively, the week which contains the 4th of January. Weeks are numbered w01 to w53. w00 for NQRustBackup is the week that precedes the first ISO week (i.e. has the first few days of the year if any occur before Thursday). w00 is not defined by the ISO specification. A week starts with Monday and ends with Sunday.

According to the NIST (US National Institute of Standards and Technology), 12am and 12pm are ambiguous and can be defined to anything. However, 12:01am is the same as 00:01 and 12:01pm is the same as 12:01, so NQRustBackup defines 12am as 00:00 (midnight) and 12pm as 12:00 (noon). You can avoid this ambiguity (confusion) by using 24 hour time specifications (i.e. no am/pm).
...
Internally NQRustBackup keeps a schedule as a bit mask. There are six masks and a minute field to each schedule. The masks are hour, day of the month (mday), month, day of the week (wday), week of the month (wom), and week of the year (woy). The schedule is initialized to have the bits of each of these masks set, which means that at the beginning of every hour, the job will run. When you specify a month for the first time, the mask will be cleared and the bit corresponding to your selected month will
be selected. If you specify a second month, the bit corresponding to it will also be added to the mask. Thus when NQRustBackup checks the masks to see if the bits are set corresponding to the current time, your job will run only in the two months you have set. Likewise, if you set a time (hour), the hour mask will be cleared, and the hour you specify will be set in the bit mask and the minutes will be stored in the minute field.
...
will cause NQRustBackup to automatically create a new FileSet
...
NQRustBackup will ensure that the next backup is always a full backup.
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-fileset-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-fileset-description.rst.inc
...
.. config:option:: dir/fileset/include/File
   ...
   .. note::
      ...
      Windows users, please take note to specify directories (even :file:`c:/...`) in Unix path notation. If you use Windows conventions, you will most likely not be able to restore your files due to the fact that the Windows path separator was defined as an escape character long before Windows existed, and NQRustBackup adheres to that convention (i.e. means the next character appears as itself).
   ...
   NQRustBackup’s default for processing directories is to recursively descend in the directory saving all files and subdirectories. NQRustBackup will not by default cross filesystems (or mount points in Unix parlance). This means that if you specify the root partition (e.g. :file:`/`), NQRustBackup will save only the root partition and not any of the other mounted filesystems. Similarly on Windows systems, you must explicitly specify each of the drives you want saved (e.g. :file:`c:/` and :file:`d:/` ...). In addition, at least for
   ...
   Take special care not to include a directory twice or NQRustBackup will by default backup the same files two times wasting a lot of space on your archive device. Including a directory twice is very easy to do. For example:
   ...
   Using the directive :config:option:`dir/fileset/include/options/Shadowing` NQRustBackup can be configured to detect and exclude duplicates automatically.
   ...
   ``File = "|command-server"``
      Any name beginning with a vertical bar (|) is assumed to be the name of a program. This program will be executed on the Director’s machine at the time the Job starts (not when the Director reads the configuration file), and any output from that program will be assumed to be a list of files or directories, one per line, to be included. Before submitting the specified command NQRustBackup will perform :ref:`character substitution <character substitution>`.
      ...
      In general, you will need to prefix your command or commands with a :command:`sh -c` so that they are invoked by a shell. This will not be the case if you are invoking a script as in the second example below. Also, you must take care to escape (precede with a ``\``) wild-cards, shell character, and to ensure that any spaces in your command are escaped as well. If you use a single quotes (’) within a double quote ("), NQRustBackup will treat everything between the single quotes as one field so it will not be necessary to escape the spaces. In general, getting all the quotes and escapes correct is a real pain as you can see by the next example. As a consequence, it is often easier to put everything in a file and simply use the file name within NQRustBackup. In that case the sh -c will not be necessary providing the first line of the file is #!/bin/sh.
      ...
      will produce a list of all the local partitions on a Linux system. Quoting is a real problem because you must quote for NQRustBackup which consists of preceding every \\ and every " with a \\, and you must also quote for the shell command. In the end, it is probably easier just to execute a script file with:
   ...
   Raw Partition
      ...
      If you explicitly specify a block device such as :file:`/dev/hda1`, then NQRustBackup will assume that this is a raw partition to be backed up. In this case, you are strongly urged to specify a Sparse=yes include option, otherwise, you will save the whole partition rather than just the actual data that the partition contains. For example:
      ...
      will backup the data in device :file:`/dev/hd6`. Note, :file:`/dev/hd6` must be the raw partition itself. NQRustBackup will not back it up as a raw device if you specify a symbolic link to a raw device such as my be created by the LVM Snapshot utilities.
...
.. config:option:: dir/fileset/include/ExcludeDirContaining
   ...
   then NQRustBackup will not backup the two directories named:
...
Once NQRustBackup determines that the Options resource matches the file under consideration, that file will be saved without looking at any other Options resources that may be present. This means that any wild cards must appear before an Options resource without wild cards.

If for some reason, NQRustBackup checks all the Options resources to a file under consideration for backup, but there are no matches (generally because of wild cards that don’t match), NQRustBackup as a default will then backup the file. This is quite logical if you consider the case of no Options clause is specified, where you want everything to be backed up, and it is important to keep in mind when excluding as mentioned above.

However, one additional point is that in the case that no match was found, NQRustBackup will use the options found in the last Options resource. As a consequence, if you want a particular set of "default" options, you should put them in an Options resource after any other Options.
...
.. config:option:: dir/fileset/include/options/compression
   ...
   However, using NQRustBackup software compression and device hardware compression together
   ...
   LZO
        ...
        Note that NQRustBackup only use one compression level LZO1X-1 specified by LZO.
...
.. config:option:: dir/fileset/include/options/Signature
   ...
   The XXH128 algorithm is not cryptographically safe, but it is suitable for non-cryptographic purposes (like calculating a checksum to avoid avoid data corruption as used by NQRustBackup here).  NQRustBackup suggests XXH128 as the preferred algorithm due to the fact that it has by magnitudes lower computational requirements. The calculation of the cryptographical checksum like MD5 or SHA has proven to be the bottleneck in environments with high-speed requirements.
...
.. config:option:: dir/fileset/include/options/OneFs
   ...
   If set to :strong:`yes`, NQRustBackup will remain on a single
   ...
   NQRustBackup will inform you when it decides not to
   ...
   to :strong:`no`, NQRustBackup will backup  all mounted file systems (i.e. traverse mount
   ...
   If you think that NQRustBackup should be backing up a particular directory
   ...
   NQRustBackup is processing the :file:`/` directory.  This message does not
   indicate an error. This message means that while examining the
   :strong:`File =` referred to in the second part of the message, NQRustBackup will
   ...
   message saying that NQRustBackup will not descend from :file:`/` into :file:`/var`.  But
   it is important to realise that NQRustBackup will descend into :file:`/var` from the
...
.. config:option:: dir/fileset/include/options/HonorNoDumpFlag
   ...
   BSD-derived systems) NQRustBackup will honor the setting of the flag
...
.. config:option:: dir/fileset/include/options/portable
   ...
   If set to :strong:`yes` (default is :strong:`no`), the NQRustBackup File daemon will
...
.. config:option:: dir/fileset/include/options/recurse
   ...
   If set to :strong:`yes` (the default), NQRustBackup will recurse (or descend) into
   all subdirectories found unless the directory is explicitly excluded
   using an :strong:`exclude` definition.  If you set :strong:`recurse=no`, NQRustBackup
...
.. config:option:: dir/fileset/include/options/sparse
   ...
   :strong:`Restrictions:` NQRustBackup reads files in 32K buffers.  If the whole
   ...
   4098 bytes) that are all zero.  As a consequence, NQRustBackup's detection of
   ...
   :strong:`sparse` option, NQRustBackup will specifically look for empty space in
   the file, and any empty space will not be written to the Volume, nor
   will it be restored.  The price to pay for this is that NQRustBackup must
...
.. config:option:: dir/fileset/include/options/ReadFifo
   ...
   is not the case, NQRustBackup will hang indefinitely on reading/writing the
   FIFO. When this is not enabled (default), the Client simply saves the
   directory entry for the FIFO.

   Normally, when NQRustBackup runs a RunBeforeJob, it waits until that
   script terminates, and if the script accesses the FIFO to write
   into it, the NQRustBackup job will block and everything will stall.
   ...
   it to the fifo.  NQRustBackup will then write it to the Volume.

   During the restore operation, the inverse is true, after NQRustBackup creates
   ...
   you must ensure that there is a reader program or NQRustBackup will block, and
   after one minute, NQRustBackup will time out the write to the fifo and move on
   to the next file.
...
.. config:option:: dir/fileset/include/options/NoAtime
   ...
   open flag, NQRustBackup will open all files to be backed up with this option.
   ...
   file integrity checkers (and NQRustBackup can fit on both categories).
   ...
   silently ignored by NQRustBackup.
...
.. config:option:: dir/fileset/include/options/KeepAtime
   ...
   The default is :strong:`no`.  When enabled, NQRustBackup will reset the st\_atime
   ...
   However, for some files, such as mailboxes, when NQRustBackup backs up the
   file, the user will notice that someone (NQRustBackup) has accessed the
   file. In this, case keepatime can be useful.
   (I'm not sure this works on Win32).

   Note, if you use this feature, when NQRustBackup resets the access time, the
...
.. config:option:: dir/fileset/include/options/HardLinks
   ...
   When disabled, NQRustBackup will backup each file individually and restore them as
   ...
   .. note::
      ...
      with the contents attached, then NQRustBackup will not restore its data, but
...
.. config:option:: dir/fileset/include/options/Wild
   ...
   backup by using the :ref:`nqrustbackup_wild` program.
...
.. config:option:: dir/fileset/include/options/Regex
   ...
   backup by using the :ref:`nqrustbackup_regex` program.
   You can also test your full FileSet definition by using
   the :ref:`estimate <estimate>` command.
...
.. config:option:: dir/fileset/include/options/AclSupport
   ...
   POSIX :strong:`libacl` installed on your Linux system, NQRustBackup will backup the
   ...
   available on Unix systems only and requires the Linux ACL library. NQRustBackup is
   ...
   files NQRustBackup will try to restore the ACLs, if there is no ACL support
   available on the system, NQRustBackup restores the files and directories but
...
.. config:option:: dir/fileset/include/options/XAttrSupport
   ...
   Extensible Attributes NQRustBackup will backup the file and directory
   ...
   (Linux, Darwin, FreeBSD) NQRustBackup checks if you have the aclsupport
...
.. config:option:: dir/fileset/include/options/FsType
   ...
   so that NQRustBackup will traverse filesystems.
...
.. config:option:: dir/fileset/include/options/Meta
   ...
   variables via the Data Management Agent (DMA) in NQRustBackup to the remote
...
The following is an example of a valid FileSet resource definition. Note, the first Include pulls in the contents of the file :file:`/etc/backup.list` when NQRustBackup is started (i.e. the @), and that file must have each filename to be backed up preceded by a File = and on a separate line.
...
Now lets take a slight variation on the above and suppose you want to save all your whole filesystem except :file:`/tmp`. The problem that comes up is that NQRustBackup will not normally cross from one filesystem to another. Doing a :command:`df` command, you get the following output:
...
And we see that there are a number of separate filesystems (/ /boot /home /rescue /tmp and /usr not to mention mounted systems). If you specify only / in your Include list, NQRustBackup will only save the Filesystem /dev/hda5. To save all filesystems except /tmp with out including any of the Samba or NFS mounted systems, and explicitly excluding a /tmp, /proc, .journal, and .autofsck, which you will not want to be saved and restored, you can use the following:
...
Now, lets assume you only want to backup .Z and .gz files and nothing else. This is a bit trickier because NQRustBackup by default will select everything to backup, so we must exclude everything but .Z and .gz files. If we take the first example above and make the obvious modifications to it, we might come up with a FileSet that looks like this:
...
NQRustBackup uses the system regex library and some of them are different on different OSes.
...
Please be aware that allowing NQRustBackup to traverse or change file systems can be very dangerous. For example, with the following:
...
you will be backing up an NFS mounted partition (/mnt/matou), and since onefs is set to no, NQRustBackup will traverse file systems. Now if /mnt/matou has the current machine’s file systems mounted, as is often the case, you will get yourself into a recursive loop and the backup will never end.
...
Firstly, NQRustBackup walks over the filesystem depth-first starting from the File = lines. It stops descending when a directory is excluded, so you must include all ancestor directories of each directory containing files to be included.
...
On Win32 systems, if you move a directory or file or rename a file into the set of files being backed up, and a Full backup has already been made, NQRustBackup will not know there are new files to be saved during an Incremental or Differential backup (blame Microsoft, not us). To avoid this problem, please copy any new directory or files into the backup area. If you do not have enough disk to copy the directory or files, move them, but then initiate a Full backup.
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-client-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-client-description.rst.inc

The following is an example of a valid Client resource definition:

.. code-block:: nqrustbackupconfig
   :caption: Minimal client resource definition in nqrustbackup-dir.conf
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-storage-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-storage-description.rst.inc
...
If automatic labeling is not enabled (see :ref:`AutomaticLabeling`) the physical media must be manually labeled. The labeling can either be done with the :bcommand:`label` command in the console program or using the :command:`nqrustbackup_tape` program. The preferred method is to use the :bcommand:`label` command in the console program. Generally, automatic labeling is enabled for :config:option:`sd/device/DeviceType = File`\
and disabled for :config:option:`sd/device/DeviceType = Tape`\ .

Finally, you must add Volume names (and their attributes) to the Pool. For Volumes to be used by NQRustBackup they must be of the same :config:option:`sd/device/MediaType`\  as the archive device specified for the job (i.e. if you are going to back up to a DLT device, the Pool must have DLT volumes defined since 8mm volumes cannot be mounted on a DLT drive). The :config:option:`sd/device/MediaType`\  has particular importance if you are backing up to files.
When running a Job, you must explicitly specify which Pool to use. NQRustBackup will then automatically select the next Volume to use from the Pool, but it will ensure that the :config:option:`sd/device/MediaType`\  of any Volume selected from the Pool is identical to that required by the Storage resource you have specified for the Job.
...
As previously mentioned, each time NQRustBackup starts, it scans all the Pools associated with each Catalog, and if the database record does not already exist, it will be created from the Pool Resource definition. If you change the Pool definition, you manually have to call :bcommand:`update pool` command in the console program to propagate the changes to existing volumes.

The Pool Resource defined in the Director’s configuration may contain the following directives:

.. include:: /include/autogenerated/nqrustbackup-dir-resource-pool-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-pool-description.rst.inc
...
In general, you can give your Pools any name you wish, but there is one important restriction: the Pool named Scratch, if it exists behaves like a scratch pool of Volumes in that when NQRustBackup needs a new Volume for writing and it cannot find one, it will look in the Scratch pool, and if it finds an available Volume, it will move it out of the Scratch pool into the Pool currently being used by the job.
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-catalog-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-catalog-description.rst.inc
...
.. code-block:: nqrustbackupconfig
   ...
   {
     Name = MyCatalog
     DB Name = nqrustbackup;
     DB User = nqrustbackup;
     DB Password = ""
   }
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-console-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-console-description.rst.inc

The example at :ref:`section-ConsoleAccessExample` shows how to use a console resource for a connection from a client like :command:`nqrustbackup_console`.
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-user-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-user-description.rst.inc
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-profile-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-profile-description.rst.inc
...
.. include:: /include/autogenerated/nqrustbackup-dir-resource-counter-table.rst.inc

.. include:: /include/autogenerated/nqrustbackup-dir-resource-counter-description.rst.inc
