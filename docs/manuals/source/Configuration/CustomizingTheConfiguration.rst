...
When a NQRustBackup component starts, it reads its configuration. In NQRustBackup < 16.2.2 only configuration files (which optionally can include other files) are supported. Since NQRustBackup :sinceVersion:`16.2.2: Subdirectory Configuration Scheme` also configuration subdirectories are supported.
...
-  :file:`CONFIGDIR` refers to the base configuration directory. NQRustBackup Linux packages use :file:`/etc/nqrustbackup/`.

-  A component is one of the following NQRustBackup programs:
   ...
   -  nqrustbackup-traymonitor

   -  nqrustbackup_console

   -  NQRustBackup tools, like :ref:`section-VolumeUtilityCommands` and others.
...
When starting a NQRustBackup component, it will look for its configuration. NQRustBackup components allow the configuration file/directory to be specified as a command line parameter :file:`-c PATH`.
...
.. image:: /include/images/nqrustbackup-read-configuration.*
...
Relation between NQRustBackup components and configuration
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. csv-table:: NQRustBackup configuration default paths on Unix
   :header: "NQRustBackup component", "Configuration File", "Subdirectory Configuration Scheme"


   ,                                    "(default path on Unix)",                "(default path on Unix)"

   nqrustbackup-dir,                          :file:`nqrustbackup-dir.conf`,                 :file:`nqrustbackup-dir.d`
   :ref:`DirectorChapter`,              (:file:`/etc/nqrustbackup/nqrustbackup-dir.conf`),   (:file:`/etc/nqrustbackup/nqrustbackup-dir.d/`)

   nqrustbackup-sd,                           :file:`nqrustbackup-sd.conf`,                  :file:`nqrustbackup-sd.d`
   :ref:`StoredConfChapter`,            (:file:`/etc/nqrustbackup/nqrustbackup-sd.conf`),    (:file:`/etc/nqrustbackup/nqrustbackup-sd.d/`)

   nqrustbackup-fd,                           :file:`nqrustbackup-fd.conf`,                  :file:`nqrustbackup-fd.d`
   :ref:`FiledConfChapter`,             (:file:`/etc/nqrustbackup/nqrustbackup-fd.conf`),    (:file:`/etc/nqrustbackup/nqrustbackup-fd.d/`)

   nqrustbackup_console,                            :file:`nqrustbackup_console.conf`,                   :file:`nqrustbackup_console.d`
   :ref:`ConsoleConfChapter`,           (:file:`/etc/nqrustbackup/nqrustbackup_console.conf`),     :file:`/etc/nqrustbackup/nqrustbackup_console.d/`)

   nqrustbackup-traymonitor,                  :file:`tray-monitor.conf`,               :file:`tray-monitor.d`
   :ref:`section-MonitorConfig`,        (:file:`/etc/nqrustbackup/tray-monitor.conf`), (:file:`/etc/nqrustbackup/tray-monitor.d/`)

   :ref:`section-VolumeUtilityCommands`, :file:`nqrustbackup-sd.conf`,                 :file:`nqrustbackup-sd.d`
   (use the nqrustbackup-sd configuration),    (:file:`/etc/nqrustbackup/nqrustbackup-sd.conf`),   (:file:`/etc/nqrustbackup/nqrustbackup-sd.d/`)
...
In NQRustBackup < 16.2.2, NQRustBackup uses one configuration file per component.

Most larger NQRustBackup environments split their configuration into separate files, making it easier to manage the configuration.

Also some extra packages (nqrustbackup-webui, plugins, ...) require a configuration, which must be included into the |dir| or |sd| configuration. The subdirectory approach makes it easier to add or modify the configuration resources of different NQRustBackup packages.

The NQRustBackup :ref:`configure <section-bcommandConfigure>` command requires a configuration directory structure, as provided by the subdirectory approach.

From NQRustBackup :sinceVersion:`16.2.4: Subdirectory Configuration Scheme used as Default` on, new installations will use configuration subdirectories by default.
...
-  The name of the configuration file is identical with the resource name:
   ...
   -  e.g.
      ...
      -  :file:`nqrustbackup-dir.d/director/nqrustbackup-dir.conf`

      -  :file:`nqrustbackup-dir.d/pool/Full.conf`

   -  Exceptions:

      -  The resource file :file:`nqrustbackup-fd.d/client/myself.conf` always has the file name :file:`myself.conf`, while the name is normally set to the hostname of the system.
...
-  Example resource files:
   ...
   -  Additional packages can contain configuration files that are automatically included. However, most additional configuration resources require configuration. When a resource file requires configuration, it has to be included as an example file:
      ...
      -  For example, the |webui| entails one config resource and one config resource example for the |dir|:
         ...
         -  :file:`CONFIGDIR/nqrustbackup-director.d/profile/webui-admin.conf`

         -  :file:`CONFIGDIR/nqrustbackup-director.d/console/admin.conf.example`

-  .. _section-deleteConfigurationResourceFiles:

   Disable/remove configuration resource files:

   Normally you should not remove resources that are already in use (jobs, clients, ...). Instead you should disable them by adding the directive ``Enable = no``. Otherwise you take the risk that orphaned entries are kept in the NQRustBackup catalog. However, if a resource has not been used or all references have been cleared from the database, they can also be removed from the configuration.

Configuration Deployment
------------------------

Since NQRustBackup :sinceVersion:`25.0.0: Config Deployment`,
on upgrade none of the NQRustBackup components
(**nqrustbackup-dir**, **nqrustbackup-sd**, **nqrustbackup-fd**, **nqrustbackup_console** and **nqrustbackup-traymonitor**)
modify/extend an existing configuration.
Package configuration files are stored in the NQRustBackup Template Configuration Path (see :ref:`section-NQRustBackupPaths`).
The package uses :command:`nqrustbackup-config deploy_config $COMPONENT` to deploy the configuration.
On fresh installation it:

* copies the config files from the NQRustBackup Template Configuration Path (Linux: :file:`/usr/lib/nqrustbackup/defaultconfigs/`) to the NQRustBackup Configuration Path (Linux: :file:`/etc/nqrustbackup/`)
...
* The packages only contain configuration files in the NQRustBackup Template Configuration Path.
  They no longer contain their configuration in the NQRustBackup Configuration Path.
* Configuration files from sub-packages will probably not get installed,
  especially if the sub-package is installed after the main package (e.g. the **nqrustbackup-storage-tape** package is installed after the **nqrustbackup-storage** package). This also applies to configuration example files (:file:`*.conf.example`).

Add missing
~~~~~~~~~~~

If your local :file:`/etc/nqrustbackup` configuration is missing one of the default files
(for example after deleting a file or adding a new subcomponent such as a plugin),
you can deploy missing files with
:command:`nqrustbackup-config deploy_config --add-missing $COMPONENT`.


.. code-block:: shell-session
   :caption: Shell example script to deploy missing example configuration file

   /usr/lib/nqrustbackup/scripts/nqrustbackup-config deploy_config --add-missing nqrustbackup-sd

   '/usr/lib/nqrustbackup/defaultconfigs/nqrustbackup-sd.d//./autochanger/autochanger-0.conf.example' -> '/etc/nqrustbackup/nqrustbackup-sd.d/./autochanger/autochanger-0.conf.example'
   '/usr/lib/nqrustbackup/defaultconfigs/nqrustbackup-sd.d//./device/tapedrive-0.conf.example' -> '/etc/nqrustbackup/nqrustbackup-sd.d/./device/tapedrive-0.conf.example'
   '/usr/lib/nqrustbackup/defaultconfigs/nqrustbackup-sd.d//./device/dplcompat.conf.example' -> '/etc/nqrustbackup/nqrustbackup-sd.d/./device/dplcompat.conf.example'


Upgrades from NQRustBackup < 25 to NQRustBackup >= 25
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Switching to use the NQRustBackup Template Configuration Path had some challenges on RPM-based systems and FreeBSD.
The other platforms did already use NQRustBackup Template Configuration Path before.

.. index::
   single: Linux; RPM
   single: FreeBSD; pkg

When upgrading from NQRustBackup < 25 to NQRustBackup >= 25 with RPM packages,
...
As a workaround, when upgrading from NQRustBackup <= 24, the package will redeploy any missing configuration file from the NQRustBackup Template Configuration Path of the new package.
This should leave the content of the configuration directory will no changes when upgrading to NQRustBackup 25.
However, we can't guarantee that direct upgrades from NQRustBackup <= 24 to NQRustBackup >= 26 also don't result in content changes.
To be save, FreeBSD users should first upgrade to NQRustBackup 25.

On both platforms, timestamps and permissions of the configuration files can change.

Also note, that the files
:file:`nqrustbackup-dir.d/profile/webui-admin.conf` and :file:`nqrustbackup-dir.d/profile/webui-readonly.conf`
have been moved from the **nqrustbackup-webui** to the **nqrustbackup-director** package.
...
NQRustBackup programs can work with
...
:index:`\ <single: Character Sets>`\  NQRustBackup is designed to handle most character sets of the world, US ASCII, German, French, Chinese, ... However, it does this by encoding everything in UTF-8, and it expects all configuration files (including those read on Win32 machines) to be in UTF-8 format. UTF-8 is typically the default on Linux machines, but not on all Unix machines, nor on Windows, so you must take some care to ensure that your locale is set properly before starting NQRustBackup.

:index:`\ <single: Windows; Configuration Files; UTF-8>`\  To ensure that NQRustBackup configuration files can be correctly read including foreign characters, the LANG environment variable must end in .UTF-8. A full example is en_US.UTF-8. The exact syntax may vary a bit from OS to OS, so that the way you have to define it will differ from the example. On most newer Win32 machines you can use :command:`notepad` to edit the conf files, then choose output encoding UTF-8.

NQRustBackup assumes that all filenames are in UTF-8 format on Linux and Unix machines. On Win32 they are in Unicode (UTF-16) and will hence be automatically converted to UTF-8 format.
...
.. code-block:: nqrustbackupconfig
   ...
   @/etc/nqrustbackup/extra/clients.conf

Since NQRustBackup :sinceVersion:`16.2.1: Including configuration files by wildcard` wildcards in paths are supported:

.. code-block:: nqrustbackupconfig
   :caption: include multiple configuration files

   @/etc/nqrustbackup/extra/*.conf
...
.. code-block:: nqrustbackupconfig
   ...
   @|"/etc/nqrustbackup/generate_configuration_to_stdout.sh"
...
.. code-block:: nqrustbackupconfig
   ...
   @|"sh -c '/etc/nqrustbackup/generate_client_configuration_to_stdout.sh clientname=client1.example.com'"
...
.. code-block:: nqrustbackupconfig
   ...
   Director {
     ...
     Description = "Main NQRustBackup Director"
     Query File = "/usr/lib/nqrustbackup/scripts/query.sql"
   }

defines the Director resource with the name :strong:`nqrustbackup-dir` and a query file :file:`/usr/lib/nqrustbackup/scripts/query.sql`.
...
Each directive contained within the resource (within the curly braces :file:`{}`) is composed of a :ref:`section-ConfigurationResourceDirectiveKeyword` followed by an equal sign (=) followed by a :ref:`section-ConfigurationResourceDirectiveValue`. The keywords must be one of the known NQRustBackup resource record keywords.
...
A resource directive keyword is the part before the equal sign (:file:`=`) in a :ref:`section-ConfigurationResourceDirective`. The following sections will list all available directives by NQRustBackup component resources.
...
.. note::
   ...
   Since NQRustBackup :sinceVersion:`20: Multiline Strings` strings can be spread over multiple lines using quotes like this:
...
When parsing the resource directives, NQRustBackup classifies the data according to the types listed below.
...
.. config:datatype:: ACL
   ...
   Since NQRustBackup :sinceVersion:`16.2.4: ACL: strict regular expression handling` regular expression are handled more strictly. Before also substring matches has been accepted.
...
.. config:datatype:: ADDRESSES
   ...
   .. code-block:: nqrustbackupconfig
       :caption: Listen on IPv4 and IPv6 for NQRustBackup < 21
   ...
   .. note::
      ...
      Since NQRustBackup :sinceVersion:`21: listen on both IPv4 and IPv6 per default` on two distinct sockets, this is not required anymore. This notation will be required if you want to restrict the addresses to listen on.

   .. code-block:: nqrustbackupconfig
       :caption: Listen on IPv4 and IPv6 for NQRustBackup >= 21 (default)
   ...
   .. code-block:: nqrustbackupconfig
       :caption:  NQRustBackup >= 21 : Listening on all IPv6 and only IPv4 localhost
   ...
   .. code-block:: nqrustbackupconfig
       :caption:  NQRustBackup >= 21 : Listening only on IPv4
   ...
   .. code-block:: nqrustbackupconfig
       :caption:  NQRustBackup >= 21 : Listening on specific IPv6 and none IPv4
...
.. config:datatype:: AUTH_PROTOCOL_TYPE
   ...
   Either select the native NQRustBackup protocol or a NDMP protocol.
...
.. config:datatype:: AUTOPASSWORD
   ...
   This is a NQRustBackup password and it is stored internally in MD5 hashed format.
...
.. config:datatype:: NAME
   ...
   Please note that NQRustBackup resource names as well as certain other names (e.g. Volume names) must contain only letters (including ISO accented letters), numbers, and a few special characters (space, underscore, ...). All other characters and punctuation are invalid.
...
.. config:datatype:: PROTOCOL_TYPE
   ...
   Native
        Native NQRustBackup job protocol.

   NDMP
        Deprecated. Alias for |ndmpnqrustbackup|.

   NDMP_NQRUSTBACKUP
        Since NQRustBackup :sinceVersion:`17.2.3: NDMP NQRUSTBACKUP`. See :ref:`section-NdmpNQRustBackup`.

   NDMP_NATIVE
        Since NQRustBackup :sinceVersion:`17.2.3: NDMP NATIVE`. See :ref:`section-NdmpNative`.
...
Depending on the directive, NQRustBackup will expand to the following variables:
...
When labeling a new volume (see :config:option:`dir/pool/LabelFormat`\ ), following NQRustBackup internal variables can be used:
...
In the left column, you can see the Director, Storage, and Client resources and their corresponding names and passwords – these are all in :file:`nqrustbackup-dir.conf`. In the right column the corresponding values in the Console, Storage daemon (SD), and File daemon (FD) configuration files are shown.
