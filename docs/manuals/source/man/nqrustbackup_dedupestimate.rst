...
**nqrustbackup_dedupestimate** [*options*] <*devicename*>

Description
-----------

:program:`nqrustbackup_dedupestimate` reads the given volumes with the specified device and
...
`<https://docs.nqrustbackup.org/>`_

Internal workflow
-----------------
* :program:`nqrustbackup_dedupestimate` reads records from each volume one by one,
...
.. program:: nqrustbackup_dedupestimate

.. option:: devicename TEXT

   Specify the input device name (either as name of a NQRustBackup Storage Daemon
   Device resource or identical to the Archive Device in a NQRustBackup Storage Daemon
   Device resource).
