...
Block
    ...
    However, some of the Storage tools (`bls <#bls>`__, nqrustbackup_scan, nqrustbackup_extract,
...
Record Header
    ...
    Note, for Block headers of level BB02 (Bacula >= 1.27 and NQRustBackup),
...
A NQRustBackup output file consists of Blocks of data. Each block contains a
...
No Record Header will be split across NQRustBackup blocks. However, Record
Data may be split across any number of NQRustBackup blocks. Obviously this
will not be the case for the Volume Label which will always be smaller
than the NQRustBackup Block size.
...
NQRustBackup will write a short block before writing the session record
...
be fully written. If your drive permits backspace record, NQRustBackup will
backup over the last record written on the tape, re-read it and verify
that it was correctly written.

When a new tape is mounted NQRustBackup will write the full contents of the
partially written block to the new tape ensuring that there is no loss
of data. When reading a tape, NQRustBackup will discard any block that is not
totally written, thus ensuring that there is no duplication of data. In
addition, since NQRustBackup blocks are sequentially numbered within a Job, it
...
and `Label Records <#Label>`__ are written using NQRustBackup’s serialization
...
If the tape is damaged, a NQRustBackup utility will
...
NQRustBackup serialization routines and thus is guaranteed to be in machine
independent format.
...
is of variable length. The Record Header is written using the NQRustBackup
...
When reading a record, if NQRustBackup finds only part of the
...
**nqrustbackup_tape** program. Each volume is labeled with the following
information using the NQRustBackup serialization routines, which guarantee
machine byte order independence.

For NQRustBackup versions 12.4 and later, the Volume Label Format is:

::

      char Id[32] = "NQRustBackup 2.0 Immortal\n"; /* Identification */
      uint32_t VerNum;          /* Label version number; = 20 since NQRustBackup 12.4 */
...
::
      ...
      char Id[32];              /* NQRustBackup Immortal ... */
...
::
                      ...
                      NQRustBackup Tape Format
                      28 September 2002
       A NQRustBackup tape is composed of tape Blocks.  Each block
       ...
       BB02: Serves to identify the block as a
         NQRustBackup block and also serves as a block format identifier
       ...
       BlockSize: is the size in bytes of the block. When reading
         ...
         actual size read, NQRustBackup discards the block.
       ...
       Id: 32 byte identifier "NQRustBackup 2.0 immortal\n"
       ...
       VerNum: 20
       label_btime: NQRustBackup time/date tape labeled
       write_btime: NQRustBackup time/date tape first used (data written)
       ...
       VerNum: 20
       JobId: JobId
       write_btime: NQRustBackup time/date this tape record written
       PoolName: Pool Name
       PoolType: Pool Type
       MediaType: Media Type
