...
A special case of the NQRustBackup Plugins are the Python Plugins.
The Python plugins create a connection between the NQRustBackup Plugin API and the
Python programming language. With the Python plugins, it is possible to implement
NQRustBackup Plugins by writing Python code.

For each daemon there exists a Daemon Python Plugin which is a plugin implementing
the C API for NQRustBackup plugins,
...
Description of the NQRustBackup Python plugin API for NQRustBackup >= 20
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For NQRustBackup :sinceVersion:`20: Python API`, the NQRustBackup Python API was refactored and
adapted to support both Python version *2* and Python version *3*.

For NQRustBackup :sinceVersion:`23: Python API`, the support for Python version *2*
(end-of-live since January 1, 2020) was removed.

The following plugins now exist:

.. table:: NQRustBackup Python plugins
   :widths: auto
...
as real Python extension module with the name *nqrustbackup[fd|sd|dir]*, for example
**nqrustbackupfd**.
Every Python plugin now has a corresponding Python extension module.

.. uml::
  :caption: NQRustBackup Python Plugin Architecture for NQRustBackup >= 20

  package "NQRustBackup Daemon" {
  [Core]
  }
  ...
  [Core] <-> [Python Interpreter] : NQRustBackup Plugin API
...
Definitions required for the Python plugin callbacks into the NQRustBackup core
are now **compiled into** the *nqrustbackup[fd|sd|dir]* Python extension module, and the
...
The members of the **nqrustbackupfd.stat_pkt** type have been renamed with the
**st_** prefix to match with the member names of Pythons' **os.stat_result**.

Direct I/O option in Python plugin API for NQRustBackup >= 22
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The NQRustBackup Plugin API calls the plugin also to do the I/O (*read* and *write*)
...
this data back to the NQRustBackup core as ``bytearray`` on backup gets the data back in
a ``bytearray`` from the core to write it back during restore.
While this procedure implements the full NQRustBackup plugin logic, it is not very
efficient as the data needs to be moved between the python interpreter and the
NQRustBackup core and can slow down performance of backups and restores significantly.
...
core, the I/O operation now can be done by the NQRustBackup core itself by **directly
...
.. code-block:: python
   :caption: enable direct I/O in python plugins
            ...
            if I_want_io_in_core:
                ...
                IOP.status =  nqrustbackupfd.iostat_do_in_core
            else:
                #  do io in plugin
                IOP.status = nqrustbackupfd.iostat_do_in_plugin
...
In  NQRustBackup < 20, the NQRustBackup Python plugin API consists of a NQRustBackup daemon
...
exchanged via the NQRustBackup plugin interface.
...
NQRustBackup plugin implemented in Python.
...
NQRustBackup core are stored in a Python file called
*nqrustbackup_[fd|sd|dir]_consts*, for example *nqrustbackup_fd_consts.py*.

This file contained the required definitions in form of dictionaries, like the
following example:

.. code-block:: python
   :caption: nqrustbackup_fd_consts: definition as dictionary
...
.. code-block:: python
   :caption: nqrustbackup_fd_consts: accessing

   import nqrustbackup_fd_consts
   ... # more code
   return nqrustbackup_fd_consts.bRCs["bRC_OK"]


The API also always carries a **context** variable which is part of
every function call between the NQRustBackup core and Python, so that every function
being called from the core has an context which needs to be given back to every
call that goes into the core.


Porting existing Python plugins to NQRustBackup >= 20
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
and requires the following steps:
   ...
   Switch to the constants being defined in the nqrustbackup[fd|sd|dir] Python plugin.
      Import nqrustbackup[fd|sd|dir] instead of nqrustbackup_[fd|sd|dir]_consts.
