NQRustBackup Memory Management
========================
...
NQRustBackup and is meant to be a technical discussion for developers rather
than part of the user manual.

Since NQRustBackup may be called upon to handle filenames of varying and more
...
possibilities for memory usage within **NQRustBackup**. Each will be described
in turn. They are:
...
of where this is appropriate is for **NQRustBackup** resource names, which are
...
retained in a NQRustBackup memory pool. The next request to acquire pooled
...
memory is returned to the system rather than being saved in the NQRustBackup
...
NQRustBackup memory pool free chain to be used in a subsequent call for memory
from that pool.
...
of the low level NQRustBackup subroutines expect to be passed a pool memory
...
to free all unused memory retained in the NQRustBackup memory pool. Note, any
