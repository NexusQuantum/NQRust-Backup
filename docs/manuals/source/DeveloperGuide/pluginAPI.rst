NQRustBackup FD Plugin API
====================

To write a NQRustBackup plugin, you create a dynamic shared object program (or
...
**nqrustbackup-fd.conf** file in the **Client** resource, and when the FD
...
is when a particular event is detected in NQRustBackup, it will transfer
...
with NQRustBackup by getting and setting NQRustBackup variables. In this way, it
behaves much like a RunScript. Currently very few NQRustBackup variables are
...
encountered by NQRustBackup during backup, it passes the “command” part of the
...
implement a compression algorithm in a very simple way. NQRustBackup will call
...
NQRustBackup passes information to the plugin through this call and it gets
back information that it needs to use the plugin. Later, NQRustBackup will
call particular functions that are defined by the **loadPlugin**
interface.

When NQRustBackup is finished with the plugin (when NQRustBackup is going to exit),
...
definition uses the minimum code from NQRustBackup. The bc_types.h file is
required to ensure that the data type definitions in arguments
correspond to the NQRustBackup core code.
...
::
    ...
    typedef enum {
      ...
      bRC_Core  = 6,                         /* Let NQRustBackup core handles this file */
      bRC_Skip  = 7,                         /* Skip the proposed file */
    } bRC;

At a future point in time, we hope to make the NQRustBackup libbac.a into a
shared object so that the plugin can use much more of NQRustBackup’s
infrastructure, but for this first cut, we have tried to minimize the
dependence on NQRustBackup.

loadPlugin
----------

As previously mentioned, the **loadPlugin** entry point in the plugin is
called immediately after NQRustBackup loads the plugin when the File daemon
...
arguments are information from NQRustBackup that is passed to the plugin, and
the last two arguments are information about the plugin that the plugin
must return to NQRustBackup. The call is:
...
lbinfo
    This is information about NQRustBackup in general. Currently, the only
    value defined in the PluginApiDefinition structure is the version, which is the
    NQRustBackup plugin interface version, currently defined as 1. The
    **size** is set to the byte size of the structure. The exact
    ...
    ::
        ...
        typedef struct s_nqrustbackupInfo {
           uint32_t size;
           uint32_t version;
        } PluginApiDefinition;

lbfuncs
    The CoreFunctions structure defines the callback entry points within NQRustBackup
    that the plugin can use register events, get NQRustBackup values, set
    NQRustBackup values, and send messages to the Job output or debug output.

    The exact definition as of this writing is:

    ::

        typedef struct s_nqrustbackupFuncs {
           uint32_t size;
           uint32_t version;
           bRC (*registerNQRustBackupEvents)(bpContext *ctx, ...);
           bRC (*getNQRustBackupValue)(bpContext *ctx, bVariable var, void *value);
           bRC (*setNQRustBackupValue)(bpContext *ctx, bVariable var, void *value);
           bRC (*JobMessage)(bpContext *ctx, const char *file, int line,
               int type, utime_t mtime, const char *fmt, ...);
           bRC (*DebugMessage)(bpContext *ctx, const char *file, int line,
               int level, const char *fmt, ...);
           void *(*nqrustbackupMalloc)(bpContext *ctx, const char *file, int line,
               size_t size);
           void (*nqrustbackupFree)(bpContext *ctx, const char *file, int line, void *mem);
        } CoreFunctions;
...
PluginInformation
    ...
    pointer to this structure to NQRustBackup.
    ...
    version
        is the current NQRustBackup defined plugin interface version,
        currently set to 1. If the interface version differs from the
        current version of NQRustBackup, the plugin will not be run (not yet
        implemented).
    plugin_magic
    ...
    plugin_license
        ...
        NQRustBackup will only accept compatible licenses (not yet
        implemented).
    plugin_author
    ...
    The PluginInformation structure must be defined in static memory because NQRustBackup
...
PluginFunctions
    ...
    pointer to this structure to NQRustBackup. This structure contains pointer
    to each of the entry points that the plugin must provide for NQRustBackup.
    When NQRustBackup is actually running the plugin, it will call the defined
    entry points at particular times. All entry points must be defined.

    The PluginFunctions structure must be defined in static memory because NQRustBackup
    does not copy it and may refer to the values at any time while the
    plugin is loaded.
    ...
    ::
          ...
          bfuncs = lbfuncs;                  /* set NQRustBackup funct pointers */
          binfo  = lbinfo;
...
the plugin that the plugin must provide for NQRustBackup, when they are called
and their arguments. As noted above, pointers to these subroutines are
passed back to NQRustBackup in the PluginFunctions structure when NQRustBackup calls the
loadPlugin() externally defined entry point.

newPlugin(bpContext \*ctx)
~~~~~~~~~~~~~~~~~~~~~~~~~~

This is the entry point that NQRustBackup will call when a new “instance” of
...
working storage in the **pContext** variable. Note: since NQRustBackup is a
...
made by threads that correspond to a NQRustBackup job. The bpContext that will
...
::
    ...
    typedef struct s_bpContext {
      ...
      void *bContext;   /* NQRustBackup private context */
    } bpContext;

This context pointer will be passed as the first argument to all the
entry points that NQRustBackup calls within the plugin. Needless to say, the
plugin should not change the bContext variable, which is NQRustBackup’s
...
NQRustBackup will call this entry point to get a value from the plugin. This
entry point is currently not called.

setPluginValue(bpContext \*ctx, pVariable var, void \*value)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

NQRustBackup will call this entry point to set a value in the plugin. This
entry point is currently not called.

handlePluginEvent(bpContext \*ctx, bEvent \*event, void \*value)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This entry point is called when NQRustBackup encounters certain events
...
programs and scripts, and is very similar to the NQRustBackup Python
interface. When the plugin is called, NQRustBackup passes it the pointer to an
...
which defines what event has been triggered, and for each event, NQRustBackup
will pass a pointer to a value associated with that event. If no value
is associated with a particular event, NQRustBackup will pass a NULL pointer,
...
bEventRestoreCommand
    ...
    is the point at which NQRustBackup passes you the original command string
    that was specified during the backup, so you will want to save it in
    your pContext area for later use when NQRustBackup calls the plugin again.
...
bEventHandleBackupFile
    ...
    the file with NQRustBackup core functions.
...
the NQRustBackup entry points it was passed during the **loadPlugin** call and
get to a number of NQRustBackup variables. (at the current time few NQRustBackup
...
it is called when NQRustBackup encounters the “Plugin =” directive in the
Include section of the FileSet. Called when beginning the backup of a
file. Here NQRustBackup provides you with a pointer to the **save_pkt**
...
::
    ...
    struct save_pkt {
       ...
       uint32_t flags;                    /* NQRustBackup internal flags */
...
Once the backup has begun, NQRustBackup will call your plugin at the
...
In general, the sequence of commands issued from NQRustBackup to the plugin to
...
3. call NQRustBackup’s internal save_file() subroutine to save the specified
   ...
   signals to NQRustBackup that all the data was read, NQRustBackup will call the
...
the file. After this call, NQRustBackup will call pluginIO() to open the file
for write.
...
::
     ...
     enum {
       ...
       CF_CORE            /* let nqrustbackup core handles the file creation */
    };
...
directories have been sent to NQRustBackup. That allows the restore process to
...
IO_LSEEK
    ...
    Note: NQRustBackup will call IO_SEEK only when writing a sparse file.

bool checkFile(bpContext \*ctx, char \*fname)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If this entry point is set, NQRustBackup will call it after backing up all
file data during an Accurate backup. It will be passed the full filename
for each file that NQRustBackup is proposing to mark as deleted. Only files
...
NQRustBackup Plugin Entrypoints
-------------------------

When NQRustBackup calls one of your plugin entrypoints, you can call back to
the entrypoints in NQRustBackup that were supplied during the xxx plugin call
to get or set information within NQRustBackup.

bRC registerNQRustBackupEvents(bpContext \*ctx, …)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This NQRustBackup entrypoint will allow you to register to receive events that
...
available in NQRustBackup. The following Variables can be referenced:
...
Calling this entrypoint allows you to set particular values in NQRustBackup.
...
void nqrustbackupMalloc(bpContext \*ctx, const char \*file, int line, size_t size)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This call permits you to obtain memory from NQRustBackup’s memory allocator.

void nqrustbackupFree(bpContext \*ctx, const char \*file, int line, void \*mem)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This call permits you to free memory obtained from NQRustBackup’s memory
allocator.
