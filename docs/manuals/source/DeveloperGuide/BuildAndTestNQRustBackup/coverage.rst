...
NQRustBackup can be built with coverage analysis support.
...
Prerequisites
^^^^^^^^^^^^^
You should already know how to :ref:`build NQRustBackup and run tests<build-for-systemtest>`.
...
If you have a build of the sources from ``~/nqrustbackup`` in ``~/nqrustbackup/build`` you can produce a HTML report in ``~/nqrustbackup/coverage`` as follows:

.. code-block:: shell-session

   user@host:~$ gcovr \
                --html-details ~/nqrustbackup/coverage/index.html \
                --root ~/nqrustbackup \
                ~/nqrustbackup/build
...
.. code-block:: shell-session
   ...
   user@host ~$ git clone https://github.com/nqrustbackup/nqrustbackup.git
   Cloning into 'nqrustbackup'...
   remote: Enumerating objects: 113830, done.
   [...]
   user@host ~$ cd nqrustbackup
   user@host nqrustbackup$ mkdir build
   user@host nqrustbackup$ cd build
   user@host build$ cmake -Dclient-only=yes -Dcoverage=yes ..
   Entering /home/user/nqrustbackup
   -- Found Git: /usr/bin/git (found version "2.27.0")
   -- Using version information from Git
   [[...]]
   -- Configuring done
   -- Generating done
   -- Build files have been written to: /home/user/nqrustbackup/build

   user@host build$ cmake --build .
   [  0%] Building C object core/src/fastlz/CMakeFiles/nqrustbackupfastlz.dir/src/fastlzlib.c.o
   [  0%] Building CXX object core/src/plugins/filed/CMakeFiles/cephfs-fd.dir/cephfs/cephfs-fd.cc.o
   [  1%] Building C object core/src/lmdb/CMakeFiles/nqrustbackuplmdb.dir/mdb.c.o
   [[...]]
   ...
   Test project /home/user/nqrustbackup/build
   ...
   [100%] Creating HTML coverage report in /home/user/nqrustbackup/build/coverage
