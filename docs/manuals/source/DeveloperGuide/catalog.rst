...
the working details of **NQRustBackup**.
...
run by NQRustBackup, and for each Job, it must maintain a list of all files
...
The **File** table contains one entry for each file backed up by NQRustBackup.
...
To compare a NQRustBackup digest with command line tools, you have to use
...
The **Job** table contains one record for each Job run by NQRustBackup.
...
the Device resource. This record allows NQRustBackup to efficiently position
...
+------------------+-----------+-----------------------------------------+
| Recycle          | smallint  | Whether or not NQRustBackup can recycle the   |
|                  |           | Volumes: Yes, No                        |
+------------------+-----------+-----------------------------------------+
...
NQRustBackup in this database. Normally the Name is a fully qualified domain
name.
...
The **Version** table defines the NQRustBackup database version number. NQRustBackup
checks this number before reading the database to ensure that it is
compatible with the NQRustBackup binary file.
...
nqrustbackup_frombase64
~~~~~~~~~~~~~~~~~

The **nqrustbackup_frombase64** function is an ``plpgsql`` implementation of the ``FromBase64`` function in core/src/lib/base64.cc.  It essentially recovers the 64bit number from its base64 textual representation.

+-------------------+-------------------+---------------------+--------+
| Name              | Result data type  | Argument data types | Type   |
+===================+===================+=====================+========+
| nqrustbackup_frombase64 | bigint            | field text          | func   |
+-------------------+-------------------+---------------------+--------+
...
.. uml:: catalog/schema_nqrustbackup_postgresql.puml
