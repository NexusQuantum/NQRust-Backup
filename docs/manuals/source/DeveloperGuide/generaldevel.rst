NQRustBackup Developer Notes
======================

This document is intended mostly for developers and describes how you
can contribute to the NQRustBackup project and the general framework of making
NQRustBackup source changes.

History
-------

NQRustBackup is a fork of the open source project Bacula version 5.2. In 2010
...
branch was later on the base of NQRustBackup and since then was enriched by a
lot of new features.
...
the fork) and NQRustBackup in our AUTHORS file.

Developer Tools
----------------------

NQRustBackup provides developer tools to ensure code quality and simplify tasks like backporting, merging, and releasing changes.
These tools help enforce coding standards, detect issues, and automate parts of the release process.
For more details, see the devtools/README.md file in the NQRustBackup source repository.

Contributions
-------------

Contributions to the NQRustBackup project come in many forms: ideas,
participation in helping people on the `nqrustbackup-users`_ email list,
packaging NQRustBackup binaries for the community, helping improve the
...
Please make sure to use the NQRustBackup `Automatic Sourcecode Formatting`_.
Don’t forget any Copyrights and acknowledgments if it isn’t 100% your code.
Also, include the NQRustBackup copyright notice that can be found in every source file.

Issue Database
--------------

We use `github issues`_ as the NQRustBackup issue tracker.
Before 2024 we've used https://bugs.nqrustbackup.org/\ ,
...
the user a tip, but please politely refer him to the manual, the `nqrustbackup-users`_
mailing list and maybe the `commercial support`_.

.. _nqrustbackup-users:       https://groups.google.com/forum/#!forum/nqrustbackup-users
.. _commercial support: https://www.nqrustbackup.com/product/support/
.. _github issues:      https://github.com/nqrustbackup/nqrustbackup/issues/
...
Most of these `NQRustBackup Developer Notes`_, maybe including this section, should
...
directory of the nqrustbackup repo.
...
The NQRustBackup project has bundled its automatic sourcecode formatting into one tool: ``nqrustbackup-check-sources``.
https://github.com/nqrustbackup/nqrustbackup/blob/master/devtools/README.md describes how to use it. In short:
...
.. code-block:: shell-session
   ...
   $ pipenv sync
   $ pipenv shell
   (pip-tools)$ nqrustbackup-check-sources --since-merge --diff
   (pip-tools)$ nqrustbackup-check-sources --since-merge --modify
...
.. _GitHub discussion:         https://github.com/nqrustbackup/nqrustbackup/discussions
...
don't use the nqrustbackup replacements for C string functions.
