...
This document describes the TCP/IP protocol used by NQRustBackup to
...
NQRustBackup’s network protocol is basically a “packet oriented” protocol
...
\*fatal) bnet_open(), if successful, returns the NQRustBackup sock descriptor
...
Within NQRustBackup, we have established the convention that any time a single
...
The following diagram shows the activity on a NQRustBackup TCP server (i.e. a |dir| or a |sd|) when a TCP client initiates a network connection. The connection protocol has two possible handshake modes. Cleartext first or TLS first. Depending on the configuration, the NQRustBackup version and the type of client (i.e. |fd| or |nqrustbackup_console|) the handshake mode will be chosen. See chapter :ref:`section-TlsTechnicalDocumentation` for detailed information.

.. uml::
  :caption: Network startup activity (NQRustBackup TCP-Server)
