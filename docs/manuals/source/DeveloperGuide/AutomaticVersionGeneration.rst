...
What is this about?
-------------------
When we develop NQRustBackup we usually build and test each and every commit.
...
Whenever you run ``cmake`` to configure your source tree for building there is a CMake module that will try to determine the NQRustBackup version and fail if it cannot determine the version information.
...
NQRustBackupVersion.cmake
~~~~~~~~~~~~~~~~~~~
CMake will try to load the module NQRustBackupVersion.
...
.. _section-generating-nqrustbackupversion:

Generating NQRustBackupVersion.cmake
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
You can generate the NQRustBackupVersion module using the command ``cmake -P write_version_files.cmake`` from a git checkout.
A file called :file:`NQRustBackupVersion.cmake` will then be written to :file:`core/cmake` and :file:`webui/cmake`.

Git Metadata
~~~~~~~~~~~~
If there is no NQRustBackupVersion module, CMake will try to call git to determine the required version information.
...
~~~~~~~~~~~~~~~~~~~~
If the directory you're trying to build from is not a git repository and does not contain :file:`NQRustBackupVersion.cmake` in :file:`core/cmake` and :file:`core/webui`, then something in the process of retrieving the sources was flawed.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
In some rare cases it might be possible that you cloned your NQRustBackup sources from a repository where the work-in-progress tags are missing or you simply skipped fetching the tags.
...
If you have cloned from a fork that is missing the tags you will need to add the NQRustBackup GitHub repository as a remote and fetch the tags from there.
You can do this by ``git remote add upstream https://github.com/nqrustbackup/nqrustbackup.git`` followed by ``git fetch --tags upstream``.
...
Either build in the top directory or :ref:`generate NQRustBackupVersion.cmake <section-generating-nqrustbackupversion>` so information is not retrieved from git.

Last resort
~~~~~~~~~~~
If all else fails, you can manually generate :file:`NQRustBackupVersion.cmake` that sets VERSION_STRING and VERSION_TIMESTAMP and copy it to :file:`core/cmake` and :file:`webui/cmake`.

.. code-block:: cmake
    :caption: cmake/NQRustBackupVersion.cmake
...
Please consider opening a bug report or describe what you were doing on the nqrustbackup-users mailinglist, so we can figure out how to make it work for you.
