...
new GUI interface to **NQRustBackup** or who want to script commands to the director
...
``dbcheck`` and ``nqrustbackup_scan``). Therefore also user level security and
...
Interfacing to an interactive program such as NQRustBackup can be very
...
ways that NQRustBackup is designed to facilitate this:

-  The NQRustBackup network protocol is packet based, and thus pieces of
...
intended to be used by other NQRustBackup interface programs (GUIs).
...
the nqrustbackup_console. The output should be human readable.
...
by a new line. nqrustbackup_console does not display the end of packets (for good
reason, as some output (e.g. :bcommand:`status`) is sent in multiple packets).
If running in a nqrustbackup_console, this leads to unparsable output for human.
...
The API mode 2 (or JSON mode) has been introduced in NQRustBackup-15.2 and
...
The BVFS (NQRustBackup Virtual File System) do provide a API for browsing the
...
-  The output format for api mode 0 and 1 have changed for nqrustbackup >=
   17.2. In earlier versions the second column of the ``bvfs_lsdirs``,
   ``bvfs_lsfiles`` and ``bvfs_versions`` command have been the
   ``FilenameId``. As nqrustbackup >= 17.2 internally don’t use the
...
In nqrustbackup < 17.2 the output has been:
...
.. code-block:: nqrustbackup_console
    ...
    *.bvfs_lsfiles jobid=1 pathid=1
    1   7   1   gD OEE4 IHo B GHH GHH A G9S BAA 4 BVjBQG BVjBQG BVjBQG A A C    nqrustbackup_pluginfo
    1   4   1   gD OEE3 KH/ B GHH GHH A W BAA A BVjBQ7 BVjBQG BVjBQG A A C  nqrustbackup_regex
    ...
...
.. code-block:: nqrustbackup_console
    ...
    *.bvfs_lsdir jobid=1 pathid=1
    ...
    1   18  123 z Gli+ IHo B GHH GHH A NVkY BAA BrA BaIDUJ BaIDUJ BaIDUJ A A C  nqrustbackup-dir
    1   21  123 z GkuS IHo B GHH GHH A C1bw BAA XA BaIDUG BaIDUG BaIDUG A A C   nqrustbackup-fd
    1   19  123 z Glju IHo B GHH GHH A CeNg BAA UI BaIDUJ BaIDUJ BaIDUJ A A C   nqrustbackup-sd
    ...

    # there are a number of files in /sbin/.
    # We check, if there are different versions of the file nqrustbackup-dir.
    *.bvfs_versions jobid=0 client=nqrustbackup-fd pathid=1 fname=nqrustbackup-dir
    ...
    # multiple versions of the file nqrustbackup-dir have been backed up.
...
It provides a REST API using `fastapi <https://fastapi.tiangolo.com/>`_ and python-nqrustbackup to connect to
a NQRustBackup director. It supports the :command:`configure add` commands to create configuration for clients, jobs, filesets and some more resources. It also provides endpoints to query job information and job control (i.e. start / restart jobs, trigger restores).
...
.. code-block:: shell-session
   ...
   pip install nqrustbackup-restapi
...
Configure your NQRustBackup Server in :file:`api.ini` adapting these 3 lines of configuration:
...
.. code-block:: shell-session
   ...
   uvicorn nqrustbackup_restapi:app --reload
...
The Swagger UI contains documentation and online-tests. Use "authorize" to connect to your NQRustBackup director using a named console. Read here to learn how to configure
