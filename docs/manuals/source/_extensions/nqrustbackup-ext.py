...
def get_config_directive(text):
    ...
    if components >= 1:
        ...
        if daemon == "director" or daemon == "dir" or daemon == "nqrustbackup-dir":
            ...
            result["dmn"] = "fd"
            result["Dmn"] = "Fd"
        elif daemon == "nqrustbackup_console" or daemon == "console":
    ...
    # logger.debug('[nqrustbackup] ' + pformat(result))
...
class ConfigOptionXRefRole(XRefRole):
    ...
    def result_nodes(self, document, env, node, is_ref):
        logger = logging.getLogger(__name__)
        # logger.debug('[nqrustbackup] is_ref: {}, node[reftarget]: {}, {}'.format(str(is_ref), node['reftarget'], type(node['reftarget'])))
        # logger.debug('[nqrustbackup] ' + pformat(result))
...
def sinceVersion():
    def role(name, rawtext, text, lineno, inliner, options={}, content=[]):
        ...
        indexstring = "nqrustbackup-{}; {}".format(version, summary)
        idstring = "nqrustbackup-{}-{}".format(version, summary)
...
def downloadUrls():
    def role(name, rawtext, text, lineno, inliner, options={}, content=[]):
        urls = NQRustBackupUrls()

        distribution = ""
        if text.lower() != "none" and text != ".":
            distribution = text

        download_nqrustbackup_org_url = urls.get_download_nqrustbackup_org_url(distribution)
        download_nqrustbackup_com_url = urls.get_download_nqrustbackup_com_url(distribution)

        org_url = nodes.reference(
            rawtext, download_nqrustbackup_org_url, refuri=download_nqrustbackup_org_url, **options
        )

        if download_nqrustbackup_com_url is not None:
            text_or = nodes.Text(" or ")
            com_url = nodes.reference(
                rawtext,
                download_nqrustbackup_com_url,
                refuri=download_nqrustbackup_com_url,
                **options
            )
...
def setup(app):
    # logging.basicConfig(filename="/tmp/build/sphinx-nqrustbackup.log", level=logging.DEBUG)
    # logger = logging.getLogger(__name__)
    # logger.info("start")
    ...
    app.add_role(
        "mantis", autolink("https://bugs.nqrustbackup.org/view.php?id={}", "Issue #{}")
    )
