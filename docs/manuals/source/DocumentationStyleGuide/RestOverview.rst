...
.. literalinclude:: /DocumentationStyleGuide/example/code-block-nqrustbackupconfig.rst.inc
   :language: none

Output:

.. include:: /DocumentationStyleGuide/example/code-block-nqrustbackupconfig.rst.inc
...
Valid values for the highlighting language are:
  ...
  * ``python`` (the default)
  * ``nqrustbackupconfig`` (NQRustBackup configuration files or parts thereof)
  * ``nqrustbackup_console`` (NQRustBackup console (nqrustbackup_console) sessions)
  * ``nqrustbackuplog`` (NQRustBackup log files)
  * ``nqrustbackupmessage`` (NQRustBackup messages)
  * ``sh`` (Shell scripts)
...
There are also some roles for specific links targets, see :ref:`documentationstyleguide/nqrustbackupspecificformatting/links`.
...
In the NQRustBackup documentation we use the Sphinx extension **sphinx.ext.autosectionlabel**
...
When the Sphinx NQRustBackup Manual is edited manually,
...
Image can be included by::
   ...
   .. image:: /include/images/nqrustbackup-webui-jobs.*
      :caption: NQRustBackup WebUI: Job Overview
      :width: 80.0%
