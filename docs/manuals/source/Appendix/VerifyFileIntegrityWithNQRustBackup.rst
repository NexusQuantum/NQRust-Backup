...
Verify File Integrity with NQRustBackup
=================================

.. index::
   single: Security; Using NQRustBackup to Improve Computer
   single: Verify; File Integrity

Since NQRustBackup maintains a catalog of files, their attributes, and their signatures, it can be an
...
The **InitCatalog** level tells NQRustBackup simply to get the information on the specified files and
...
The :config:option:`dir/job/Level = Catalog`\  level tells NQRustBackup to compare the current state
...
.. note::
   ...
   You can use the other signature algorithms supported by NQRustBackup, as described in
...
.. code-block:: nqrustbackupconfig
   ...
   Client {
     Name = ClientVerify
     Address = nqrustbackup-fd.example.com
     Password = "PASSWORD"
   }
