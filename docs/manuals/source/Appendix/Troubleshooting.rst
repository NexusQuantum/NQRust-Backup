...
The NQRustBackup programs contain a lot of debug messages. Normally, these are not printed.
...
:strong:`nqrustbackup-sd1`. An example that is more likely to work: :strong:`nqrustbackup-sd1.example.com`.
...
.. code-block:: nqrustbackup_console
   ...
   *<input>resolve client=nqrustbackup-fd NONEXISTINGHOSTNAME</input>
   Connecting to Client nqrustbackup-fd at nqrustbackup:9102
   nqrustbackup-fd: Failed to resolve NONEXISTINGHOSTNAME
   *<input>resolve client=nqrustbackup-fd nqrustbackup-sd1.example.com</input>
   Connecting to Client nqrustbackup-fd at nqrustbackup:9102
   nqrustbackup-fd resolves nqrustbackup-sd1.example.com to host[ipv4;192.168.0.1]
...
they should only be used by Bacula and its replacements like NQRustBackup. However, apparently some
HP printers do use these port numbers. A :command:`ss -lntp` on the |sd|’s machine can
determine who is listening on the 9103 port (used for FD to SD communications in NQRustBackup).
...
For security reasons, NQRustBackup requires that both the |fd| and the |sd| know the name of the |dir|
...
files generated by the NQRustBackup installation process. Make only the absolutely necessary modifications
to these files – e.g. add the correct email address. Then follow the instructions in the
:ref:`Running NQRustBackup <TutorialChapter>` chapter of this manual. You will run a backup to disk and
a restore.
...
the machine. This seems to be a requirement of gethostbyname(), which is what NQRustBackup uses to
...
appropriate NQRustBackup configuration file.
...
Another thing to check is to ensure that the NQRustBackup component you are trying to access has
:strong:`Maximum Concurrent Jobs`\  set large enough to handle each of the Jobs and the Console
that want to connect simultaneously. Once the maximum connections has been reached, each NQRustBackup
...
NQRustBackup can run multiple concurrent jobs. Using the :strong:`Maximum Concurrent Jobs`\  directives,
...
complicated, consequently, restores may take longer if NQRustBackup must sort through interleaved volume
...
.. code-block:: nqrustbackupconfig
   :caption: Concurrent Jobs Example
    ...
    #
    # NQRustBackup Director Configuration /etc/nqrustbackup/nqrustbackup-dir.d/*
    #
    Director {
...
.. warning::
   ...
   ANSI/IBM labels have been deprecated as of NQRustBackup 23.
...
By default, NQRustBackup uses its own tape label (:config:option:`dir/pool/LabelType`\ ). However, NQRustBackup
...
If your are running NQRustBackup in such an environment, you must set
:config:option:`sd/device/CheckLabels`\  to yes, otherwise NQRustBackup will not recognize that these
tapes are already in use.

Writing
~~~~~~~

To configure NQRustBackup to also write ANSI/IBM tape labels, use :config:option:`dir/pool/LabelType`\  or
:config:option:`sd/device/LabelType`\ . With the proper configuration, you can force NQRustBackup to
require ANSI or IBM labels.

Even though NQRustBackup will recognize and write ANSI and IBM labels, it always writes its own tape
labels as well.

If you have labeled your volumes outside of NQRustBackup, then the ANSI/IBM label will be recognized by
NQRustBackup only if you have created the HDR1 label with NQRUSTBACKUP.DATA in the filename field (starting with
character 5). If NQRustBackup writes the labels, it will use this information to recognize the tape as a
NQRustBackup tape. This allows ANSI/IBM labeled tapes to be used at sites with multiple machines and
...
work properly with NQRustBackup using the nqrustbackup_tape program.

Get Your Tape Drive Working
~~~~~~~~~~~~~~~~~~~~~~~~~~~

In general, you should follow the following steps to get your tape drive to work with NQRustBackup. Start
...
#. Make sure that NQRustBackup (the Storage daemon) is not running or that you have unmounted the drive
...
#. Make sure you have a valid and correct Device resource corresponding to your drive. For Linux
   ...
   depending on the output from the :command:`nqrustbackup_tape` program (next item). When doing this, you
   should consult the :ref:`Storage Daemon Configuration <StoredConfChapter>` of this manual.

#. If you are using a Fibre Channel to connect your tape drive to NQRustBackup, please be sure to disable
   any caching in the NSR (network storage router, which is a Fibre Channel to SCSI converter).

#. Run the :command:`nqrustbackup_tape` test command:


   ::

         nqrustbackup_tape /dev/nst0
         test
...
#. Run the nqrustbackup_tape fill command, preferably with two volumes. This can take a long time. If you have
   an autochanger and it is configured, NQRustBackup will automatically use it. If you do not have it
...
#. Run NQRustBackup, and backup a reasonably small directory, say 60 Megabytes. Do three successive
   backups of this directory.

#. Stop NQRustBackup, then restart it. Do another full backup of the same directory. Then stop and
...
#. If you have an autochanger, you should now go back to the nqrustbackup_tape program and run the autochanger
   test:
...
#. Please set the tape drive device :config:option:`sd/device/MaximumFileSize` to a reasonable value.
   ...
   When NQRustBackup writes to tape, it adds a **file-mark** after :config:option:`sd/device/MaximumFileSize`
...
In case NQRustBackup does not work well with the Autochanger, it is preferable to "hand-test" that the
changer works. To do so, we suggest you do the following commands:

Make sure NQRustBackup is not running.

:command:`/usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 list 0 /dev/nst0 0`
...
:command:`/usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 listall 0 /dev/nst0 0`
...
:command:`/usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 transfer 1 2`
...
:command:`/usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 slots`
...
:command:`/usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 unload 1 /dev/nst0 0`
...
:command:`/usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 load 3 /dev/nst0 0`
...
:command:`/usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 loaded 0 /dev/nst0 0`
...
:command:`/usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 unload 3 /dev/nst0 0`
...
configuration, NQRustBackup should be able to operate the changer. The only remaining area of problems
will be if your autoloader needs some time to get the tape loaded after issuing the command.
After the mtx-changer script returns, NQRustBackup will immediately rewind and read the tape. If NQRustBackup
gets rewind I/O errors after a tape change, you will probably need to configure the
:strong:`load_sleep` parameter in the config file :file:`/etc/nqrustbackup/mtx-changer.conf`. You can
...
.. code-block:: sh
   ...
   #!/bin/sh
   /usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 unload 1 /dev/nst0 0
   /usr/lib/nqrustbackup/scripts/mtx-changer /dev/sg0 load 3 /dev/nst0 0
   mt -f /dev/st0 rewind
   mt -f /dev/st0 weof
...
:file:`/etc/nqrustbackup/mtx-changer.conf` to the specified value so that it will be effective when
NQRustBackup runs.
...
:file:`/etc/nqrustbackup/mtx-changer.conf` to "1".
...
It is possible to configure NQRustBackup in a way that job information is still stored in the NQRustBackup
catalog, while the individual file information is already pruned.

If all File records are pruned from the catalog for a Job, normally NQRustBackup can restore only all
files saved. That is there is no way using the catalog to select individual files. With this new
feature, NQRustBackup will ask if you want to specify a Regex expression for extracting only a part of
the full backup.
...
sequentially such as tar, but NQRustBackup repositions the tape on a block basis when restoring files
...
#. Try editing the :file:`restore.bsr` file at the Run xxx yes/mod/no prompt before starting the
   restore job and remove all the VolBlock statements. These are what causes NQRustBackup to reposition
...
#. Use :command:`nqrustbackup_extract` to extract the files you want – it reads the Volume sequentially if you
...
during a backup the tape is normally already positioned and NQRustBackup only needs to write. On the other
hand, because restoring files is done so rarely, NQRustBackup keeps only the start file and block on the
tape for the whole job rather than on a file by file basis which would use quite a lot of space in
the catalog.

NQRustBackup will forward space to the correct file mark on the tape for the Job, then forward space to
the correct block, and finally sequentially read each record until it gets to the correct one(s) for
the file or files you want to restore. Once the desired files are restored, NQRustBackup will stop reading
the tape.

Finally, instead of just reading a file for backup, during the restore, NQRustBackup must create the file,
and the operating system must allocate disk space for the file as NQRustBackup is restoring it.
...
Solution
   ...
   catalog. Note, if it is a matter of some inconsistencies in the NQRustBackup tables rather than a
   broken database, then running :ref:`nqrustbackup-dbcheck <nqrustbackup-dbcheck>` might help, but you will
...
Solution with a Catalog backup
   ...
   .. code-block:: shell-session
      ...
      su postgres -c /usr/lib/nqrustbackup/scripts/drop_nqrustbackup_tables
   ...
   After re-initializing the database, you should be able to run NQRustBackup. If you now try to use the
   ...
   .. code-block:: nqrustbackup_console
      ...
      Bootstrap:  /home/user/nqrustbackup/working/restore.bsr
   ...
   do a `make_nqrustbackup_tables` command, or you will probably erase your newly restored database tables.

Solution with a Job listing
   If you did save your database but did not make a bootstrap file, then recovering the database
   is more difficult. You will probably need to use :command:`nqrustbackup_extract` to extract the backup copy.
   ...
   ::
      ...
      22-Apr 10:23 HeadMan: NQRustBackup 14.2.8 (21Apr15): 22-Apr-2015 10:23:06
        JobId:                  7510
...
Solution without a Job Listing
   ...
   :ref:`nqrustbackup_scan <nqrustbackup_scan>` program to scan the contents of your tape into a database, which can be
...
Problem
   ...
   the JobId to restore, but NQRustBackup then reports:
...
Solution
   ...
   NQRustBackup will ask you if you would like to restore all the files in the job, and it will collect
...
Solution
   Either :command:`nqrustbackup_scan` the tape (see below for nqrustbackup_scanning), or better use :command:`bls` to find
   where it is on the tape, then use :command:`nqrustbackup_extract` to restore the database. For example,
   ...
   ::
      ...
      *query
      ...
      directory
          ...
          15: List Volumes NQRustBackup thinks are in changer
...
Solution
   ...
   from scratch and then nqrustbackup_scan in all your volumes, which is a very long, painful, and inexact
   process.

   There are basically three steps to take:

   #. Ensure that your PostgreSQL server is running and that the NQRustBackup database (normally nqrustbackup)
      exists. See the :ref:`section-CreateDatabase` chapter of the manual.

   #. Ensure that the NQRustBackup databases are created. This is also described at the above link.

   #. Start and stop the NQRustBackup Director using the appropriate |dir| configuration files (aka the
   ...
   When the above is complete, you can begin nqrustbackup_scanning your Volumes.
   Please see the :ref:`nqrustbackup_scan`\  chapter for more details.
