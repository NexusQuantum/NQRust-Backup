...
NQRustBackup plugins create virtual files to represent content in the restore tree.
...
For example, a python plugin `nqrustbackup-fd-plumbus.py` would create files in the
...
For example, python plugin `nqrustbackup-fd-plumbus.py` would create files in the
format `PLUMBUS:/actual-filename` or `PLUMBUS:/some-dir/the-filename`.
...
[Bruno Friedmann]: https://github.com/bruno-at-nqrustbackup
