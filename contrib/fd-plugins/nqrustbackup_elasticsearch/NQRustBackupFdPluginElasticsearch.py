...
# NQRustBackup python class PoC option plugin to index files
# with Tika and Elasticsearch, including NQRustBackup JobId
#
...
# 
# (c) 2019 NQRustBackup GmbH & Co. KG
# AGPL v.3
...
import os
from  NQRustBackupFdPluginBaseclass import *
...
class NQRustBackupFdPluginFileElasticsearch  (NQRustBackupFdPluginBaseclass):
    ...
    def handle_backup_file(self,context, savepkt):
        ...
        if ( savepkt.type == bFileType['FT_REG'] ):
            ...
            data['nqrustbackup_jobId'] = self.jobId
            data['nqrustbackup_fdname'] = self.fdname
            data['nqrustbackup_joblevel'] = unichr(self.level)
            data['nqrustbackup_directory'] = os.path.dirname(savepkt.fname) 
            try:
                esRes = es.index (index="nqrustbackup-test", doc_type='_doc', body=data)
            except Exception as ex:
