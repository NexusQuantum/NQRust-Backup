    ...
    Name = "client-data"
       Include  {
                Plugin = "python:"
                         ...
                         "module_name=nqrustbackup_mysql_dump:"
        }
}
```
...
```
               Plugin = "python:module_path=/usr/lib64/nqrustbackup/plugins:module_name=nqrustbackup-fd-mysql:db=test,mysql"
```
...
By default the root user (the user, which runs the NQRustBackup filedaemon) is used to connect to the database. We recommend that you set
...
```
GRANT USAGE ON *.* TO 'nqrustbackup'@'localhost' IDENTIFIED BY 'nqrustbackup';
GRANT SELECT, LOCK TABLES, SHOW VIEW, EVENT, TRIGGER ON *.* TO 'nqrustbackup'@'localhost'
```
##### dumpbinary #####
