...
import NQRustBackupFdPluginBaseclass
from lockfile import LockFile
import collections


class NQRustBackupFdPluginVz7CtFs(NQRustBackupFdPluginBaseclass.NQRustBackupFdPluginBaseclass):
    '''
    NQRustBackup FD Plugin to backup Virtuozzo container filesystems and metadata
    '''
    ...
    def __init__(self, plugindef):
        nqrustbackupfd.DebugMessage(
            ...
            (__name__, plugindef))
        events = []
        events.append(nqrustbackupfd.bEventEndRestoreJob)
        nqrustbackupfd.RegisterEvents(events)
        ...
        super(NQRustBackupFdPluginVz7CtFs, self).__init__(plugindef)
        ...
        self.config_path = ""
        self.fs_path = ""
        self.mount_basedir = "/mnt/nqrustbackup"
    ...
    def handle_plugin_event(self, event):
        ...
        '''
        if event == nqrustbackupfd.bEventEndRestoreJob:
            nqrustbackupfd.DebugMessage(
                100,
                "handle_plugin_event called with nqrustbackupfd.bEventEndRestoreJob\n")
            return self.end_restore_job()
        elif event == nqrustbackupfd.bEventEndBackupJob:
            nqrustbackupfd.DebugMessage(
                100,
                "handle_plugin_event called with nqrustbackupfd.bEventEndBackupJob\n")
            return self.end_backup_job()
        elif event == nqrustbackupfd.bEventStartBackupJob:
            nqrustbackupfd.DebugMessage(
                100, "handle_plugin_event() called with nqrustbackupfd.bEventStartBackupJob\n")
            return self.start_backup_job()
        elif event == nqrustbackupfd.bEventStartRestoreJob:
            nqrustbackupfd.DebugMessage(
                100,
                "handle_plugin_event() called with nqrustbackupfd.bEventStartRestoreJob\n")
    ...
    def mount_ploop_device(self, ):
        ...
        try:
            ...
            return False
        self.mounted = True
        nqrustbackupfd.DebugMessage(300, "ploop mount output: {}\n".format(p_mnt))
        return True
    ...
    def start_backup_file(self, savepkt):
        ...
        if not self.files:
            nqrustbackupfd.DebugMessage(
                100,
                "No files to backup\n")
            return nqrustbackupfd.bRCs['nqrustbackupfd.bRC_Stop']
        ...
        try:
            ...
            return nqrustbackupfd.bRCs['nqrustbackupfd.bRC_Skip']
        if os.path.islink(possible_link_to_dir):
            savepkt.type = nqrustbackupfd.FT_LNK
            ...
            savepkt.type = nqrustbackupfd.FT_DIREND
            savepkt.no_read = True
            savepkt.link = os.path.join(path_to_backup, "")
            savepkt.fname = path_to_backup
        elif stat.S_ISREG(osstat.st_mode):
            savepkt.type = nqrustbackupfd.FT_REG
            savepkt.fname = path_to_backup
        else:
            savepkt.type = nqrustbackupfd.FT_DELETED
            savepkt.no_read = True
            savepkt.fname = path_to_backup
        statpacket = nqrustbackupfd.StatPacket()
    ...
    def end_backup_file(self, ):
        '''
        Here we return 'nqrustbackupfd.bRC_More' as long as our list files_to_backup is not
        empty and nqrustbackupfd.bRC_OK when we are done
        '''
        nqrustbackupfd.DebugMessage(
            100,
    ...
    def start_restore_ct_job(self, ):
        ...
        try:
            nqrustbackupfd.DebugMessage(100, "CREATING PLOOP")
            ...
            for line in ploop_init.split("\n"):
                ...
                if matched:
                    ...
                    nqrustbackupfd.DebugMessage(
                        100, "MATCH: {}\n".format(
                            self.ploop_device))
            nqrustbackupfd.DebugMessage(100, "CREATED PLOOP\n")
        except subprocess.CalledProcessError:
            nqrustbackupfd.JobMessage(
                                nqrustbackupfd.M_FATAL,
    ...
    def start_restore_file(self, cmd):
        '''
        This is a workaound for wrong order of nqrustbackup events during plugin call
        It can happen that this is the first entry point on a restore
        and we missed the nqrustbackupfd.bEventStartRestoreJob event because we registerd
    ...
    def end_restore_job(self, ):
        ...
        '''
        nqrustbackupfd.DebugMessage(100, "ENTERING end_restore_job()\n")
    ...
    def end_restore_ct_job(self, ):
        ...
        # fixme: need to parse output
            ...
            nqrustbackupfd.JobMessage(nqrustbackupfd.M_ERROR,
                                "Cannot register CT '{}'\n".format(
                                self.options['name']))
            return nqrustbackupfd.bRC_Error
        nqrustbackupfd.DebugMessage(100, "LEAVING end_restore_ct_job () \n")
        return nqrustbackupfd.bRC_OK
    ...
    def create_file(self, restorepkt):
        ...
        restorepkt.create_status = nqrustbackupfd.CF_CORE
        return nqrustbackupfd.bRC_OK
    ...
    def set_file_attributes(self, restorepkt):
        ...
        if restorepkt.type == nqrustbackupfd.FT_REG or restorepkt.type == nqrustbackupfd.FT_DIREND:
