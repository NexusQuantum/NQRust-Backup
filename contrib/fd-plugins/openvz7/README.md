## NQRustBackup Plugin for OpenVZ/Virtuozzo 7 containers

This plugin allows the NQRustBackup filedaemon to backup and restore OpenVZ/Virtuozzo 7 container environments.
...
* NQRustBackup 20 is used with Python3 supported (Python2 might still work)
...
```
/usr/lib64/nqrustbackup/plugins/NQRustBackupFdPluginVz7CtFs.py:385: RuntimeWarning: Writing negative value into unsigned field
  statpacket.atime = osstat.st_atime
```

#### UTF-8 filename handling can crash NQRustBackup FD with Python 3
Since NQRustBackup 20 with Python 3 there is a bug with (NQRustBackup internal Ticket TT4200769) in the handling of bad UTF-8 encoded filenames
...
Therefore the result changed and this triggers a bug in NQRustBackup FD Core:
...
FileSet {
  ...
  Include {
    ...
    Plugin = "python3:module_path=/usr/lib64/nqrustbackup/plugins:module_name=nqrustbackup-fd-vz7_ct_fs:blocker=tmp/backup_blocker:lockfile=/tmp/nqrustbackup_backup_cts"
  }
}
```
...
restore file=/etc/resolv.conf client=testserver restoreclient=testserver restorejob=restore-files \
  pluginoptions="python:name=nqrustbackup-plugin:uuid=4b16639b-dbb9-45a5-b4c2-9f7ae90dfe2c:restore=files" \
  regexwhere=!^/mnt/nqrustbackup/2d109ae4-5231-475f-bc44-5cabf5d8503b/!/vz/root/2d109ae4-5231-475f-bc44-5cabf5d8503b/tmp/nqrustbackup-restore/! \
