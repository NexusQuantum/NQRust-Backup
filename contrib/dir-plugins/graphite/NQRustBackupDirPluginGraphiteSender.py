...
#
# GraphiteSender for NQRustBackup python plugins
# Functions taken and adapted from nqrustbackup-dir.py

from nqrustbackupdir import *
from nqrustbackup_dir_consts import *
import NQRustBackupDirPluginBaseclass
from socket import socket, AF_INET, SOCK_STREAM
import time


class NQRustBackupDirPluginGraphiteSender(NQRustBackupDirPluginBaseclass.NQRustBackupDirPluginBaseclass):
    ''' NQRustBackup DIR python plugin Nagios / Icinga  sender class '''

    def parse_plugin_definition(self, context, plugindef):
        '''
        Check, if mandatory monitoringHost is set and set default for other unset parameters
        '''
        super(NQRustBackupDirPluginGraphiteSender, self).parse_plugin_definition(
            context, plugindef)
        ...
        # we return OK in anyway, we do not want to produce NQRustBackup errors just because of failing
    ...
    def handle_plugin_event(self, context, event):
        ...
        super(NQRustBackupDirPluginGraphiteSender, self).handle_plugin_event(context, event)
    ...
    def evaluateJobStatus(self, context):
        ...
        if (self.jobStatus == 'E' or self.jobStatus == 'f'):
            self.metrics['nqrustbackup.jobs.{}.status.error'.format(job)] = 1
        elif (self.jobStatus == 'W'):
            self.metrics['nqrustbackup.jobs.{}.status.warning'.format(job)] = 1
        elif (self.jobStatus == 'T'):
            self.metrics['nqrustbackup.jobs.{}.status.success'.format(job)] = 1
            self.metrics['nqrustbackup.jobs.{}.jobbytes'.format(job)] = self.jobBytes
            self.metrics['nqrustbackup.jobs.{}.jobfiles'.format(job)] = self.jobFiles
            self.metrics['nqrustbackup.jobs.{}.runningtime'.format(job)] = self.jobRunningTime
            self.metrics['nqrustbackup.jobs.{}.throughput'.format(job)] = self.throughput
        else:
            self.metrics['nqrustbackup.jobs.{}.status.other'.format(job)] = 1
