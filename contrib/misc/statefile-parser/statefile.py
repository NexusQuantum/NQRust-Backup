...
"""This module parses a NQRustBackup filedaemon statefile.
...
of NQRustBackup, as structs are packed differently.
"""
...
def find_statefile():
    files = glob("/var/lib/nqrustbackup/nqrustbackup-fd.*.state")
    if len(files) == 1:
        return files[0]
    if len(files) == 0:
        return None
