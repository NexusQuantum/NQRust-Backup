...
#
# Copyright(C) 2015-2015 NQRustBackup GmbH & Co. KG
#
...
#
# PoC code to implement a NQRustBackup commandline tool that allows to trigger
# common tasks from a NQRustBackup client.
# bsmc stands for "NQRustBackup simple management cli"
#
...
import ConfigParser
import argparse
import nqrustbackup.bsock.bsockjson
import logging
import time
import sys
import datetime
...
if __name__ == '__main__':
    ...
    argParser = argparse.ArgumentParser(description='Graphite poller for NQRustBackup director.',
                                        prog="nqrustbackup-graphite-poller.py")
    ...
    argParser.add_argument('-c', '--config', help="Configfile", default="/etc/nqrustbackup/graphite-poller.conf")
    ...
    timeStampFile = "/var/tmp/nqrustbackup-graphite-poller_"
    ...
    try:
        logger.info("Connect to NQRustBackup director %s, host %s, port: %d" % (dirname, host, dir_port))
        # print password
        director = nqrustbackup.bsock.BSockJson(address=host, port=dir_port, dirname=dirname,
