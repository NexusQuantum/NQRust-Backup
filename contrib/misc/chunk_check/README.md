...
Install python3-nqrustbackup if not yet on your system.
...
`/usr/lib/nqrustbackup/scripts/` or `/usr/local/bin/`
...
```bash
s3cfg=$(pwd)/etc/s3cfg bucket=nqrustbackup-backups \
python3 ./chunk_check.py \
    --dir_address="localhost" \
    --dir_port=9101 \
    --console_user="admin-notls" \
    --console_pw="secret" \
    --s3cmd=/usr/lib/nqrustbackup/scripts/s3cmd-wrapper.sh \
