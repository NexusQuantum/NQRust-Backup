...
def get_arguments():
    ...
    epilog = """
    nqrustbackup-triggerjob is a Python script that allows you to perform a backup for a connected client if a definable time has passed since the last backup.
    ...
    nqrustbackup-triggerjob should be executed regularly to detect newly connected clients. To do so, a cronjob should run the script repeatedly.

    Note: nqrustbackup-triggerjob requires a connection between director and client. Therefore, activate Client Initiated Connections (https://docs.nqrustbackup.org/TasksAndConcepts/NetworkSetup.html#client-initiated-connection) to automatically establish a connection whenever possible. Otherwise no jobs will be started.

    """

    argparser = argparse.ArgumentParser(
        description="Trigger NQRustBackup jobs.", epilog=epilog
    )
