...
#
#   Copyright (C) 2024 NQRustBackup GmbH & Co. KG
#
...
#    RunsOnSuccess = No
#    RunsOnClient = No
#    Command = "/usr/lib/nqrustbackup/scripts/reschedule_job_as_full.sh '%e' '%i' '%n' '%l' '%t'"
#  }
...
if [ "${JobExitStatus}" == "Fatal Error" ] && [ "${JobLevel}" == "Incremental" ] && [ "${JobType}" == "Backup" ]; then
  if (nqrustbackup_console <<< "list joblog jobid=${JobId}" | tee -a "${debug_log_file}" | grep -q -F "new full level backup of this job is required."); then
    echo "Required new full level run will be started in 1 minute."
    if ! nqrustbackup_console <<< "run job=${JobName} level=Full when=\"$(date -d "now 1min" +"%Y-%m-%d %H:%M:%S")\" yes"; then
