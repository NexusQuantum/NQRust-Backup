...
#
#   Copyright (C) 2015-2024 NQRustBackup GmbH & Co. KG
#
...
"""
Low Level socket methods to communicate with a NQRustBackup Daemon.
"""
...
from nqrustbackup.util.nqrustbackupbase64 import NQRustBackupBase64
...
class LowLevel(object):
    ...
    @staticmethod
    def argparser_get_nqrustbackup_parameter(args):
        """Extract arguments.
        ...
        for key, value in vars(args).items():
            if value is not None:
                if key.startswith("NQRUSTBACKUP_"):
                    nqrustbackupkey = key.split("NQRUSTBACKUP_", 1)[1]
                    result[nqrustbackupkey] = value
        return result
    ...
    def connect(
        ...
        Args:
           ...
           connection_type (int): See :py:class:`nqrustbackup.bsock.connectiontype.ConnectionType`.
        ...
        Raises:
           nqrustbackup.exceptions.ConnectionError: If connection can be established.
           nqrustbackup.exceptions.PamAuthenticationError: If PAM authentication fails.
           nqrustbackup.exceptions.AuthenticationError: If NQRustBackup authentication fails.
        """
        ...
        self.name = name
        if password is None:
            raise nqrustbackup.exceptions.ConnectionError("Parameter 'password' is required.")
    ...
    def __connect(self):
        ...
        if self.tls_psk_require:
            if not self.is_tls_psk_available():
                raise nqrustbackup.exceptions.ConnectionError(
        ...
        if self.tls_psk_enable and self.is_tls_psk_available():
            ...
            except (nqrustbackup.exceptions.ConnectionError, ssl.SSLError) as e:
        ...
        if connected:
            try:
                auth = self.auth()
            except nqrustbackup.exceptions.PamAuthenticationError:
                raise
            except nqrustbackup.exceptions.AuthenticationError:
                if (
                    self.connection_type == ConnectionType.DIRECTOR
                    and self.requested_protocol_version is None
                    and self.get_protocol_version() > ProtocolVersions.nqrustbackup_12_4
                ):
                    # reconnect and try old protocol
                    self.logger.warning(
                        "Failed to connect using protocol version {0}. Trying protocol version {1}. ".format(
                            self.get_protocol_version(), ProtocolVersions.nqrustbackup_12_4
                        )
                    )
                    self.close()
                    self.__connect_plain()
                    self.protocol_messages.set_version(ProtocolVersions.nqrustbackup_12_4)
                    auth = self.auth()
                else:
                    raise
    ...
    def __connect_tls_psk(self):
        ...
        if isinstance(self.password, Password):
            ...
            raise nqrustbackup.exceptions.ConnectionError("No password provided.")
    ...
    def get_tls_psk_identity(self):
        """NQRustBackup TLS-PSK excepts the identity is a specific format."""
        name = str(self.name)
    ...
    def get_protocol_version(self):
        """Get the NQRustBackup Console protocol version that is used.

        Returns:
           int: Number that represents the NQRustBackup Console protocol version (see :py:class:`nqrustbackup.bsock.protocolversions.ProtocolVersions`.)
        """
    ...
    def auth(self):
        """
        Login to a NQRustBackup Daemon.

        Returns:
           bool: True, if the authentication succeeds. In earlier versions, authentication failures returned False. However, now an authentication failure raises an exception.

        Raises:
           nqrustbackup.exceptions.AuthenticationError: if authentication fails.
        """
        ...
        except nqrustbackup.exceptions.SignalReceivedException as e:
            self._handleSocketError(e)
            raise nqrustbackup.exceptions.AuthenticationError(
                "Received unexcepted signal: {0}".format(str(e))
            )
        if not result:
            raise nqrustbackup.exceptions.AuthenticationError("failed (in response)")
        if not self._cram_md5_challenge(
            ...
            raise nqrustbackup.exceptions.AuthenticationError("failed (in challenge)")
        ...
        if self.max_reconnects > 0:
            try:
                ...
                if self.__connect() and self._init_connection():
                    result = True
            except (socket.error, nqrustbackup.exceptions.ConnectionLostError):
                self.logger.warning("failed to reconnect")
        return result

    def call(self, command):
        """Call a NQRustBackup command.
    ...
    def _send_a_command_and_receive_result(self, command):
        ...
        Raises:
            nqrustbackup.exceptions.SocketEmptyHeader:
                if an empty header is received.
            nqrustbackup.exceptions.ConnectionLostError:
    ...
    def recv_bytes(self, length, timeout=10):
        ...
        while length > 0:
            ...
            if len(submsg) == 0:
                ...
                raise nqrustbackup.exceptions.ConnectionLostError(errormsg)
            length -= len(submsg)
            msg += submsg
        return msg
    ...
    def recv(self):
        ...
        header (4 bytes): if
            ...
            * < 0: NQRustBackup signal

        Returns:
           bytearray: Message retrieved via the connection.

        Raises:
            nqrustbackup.exceptions.SignalReceivedException:
                If a NQRustBackup signal is received.
        """
        self.__check_socket_connection()
        ...
        if header <= 0:
            ...
            raise nqrustbackup.exceptions.SignalReceivedException(header)
        # get the message
        length = header
            ...
            while True:
                ...
                try:
                    ...
                    if header <= 0:
                        ...
                        # NQRustBackup indicates end of command result by line starting with 4 digits
                        if match:
                            self.logger.debug(
    ...
    def interactive(self):
        ...
        while command != "exit" and command != "quit" and self.is_connected():
            ...
            except nqrustbackup.exceptions.JsonRpcErrorReceivedException as exp:
                print(str(exp))
    ...
    def is_end_of_message(self, data):
        """Checks if a NQRustBackup signal indicates the end of a message.
    ...
    def _cram_md5_challenge(
        ...
        # get the timestamp
        # here is the console
        # to confirm the director so can do this on nqrustbackup_console`way
        ...
        bbase64compatible = NQRustBackupBase64().string_to_base64(
            bytearray(hmac_md5.digest()), True
        )
        bbase64notcompatible = NQRustBackupBase64().string_to_base64(
        ...
        # check against compatible base64 and NQRustBackup specific base64
        if is_correct:
    ...
    def _cram_md5_respond(self, password, tls_remote_need=0, compatible=True):
        ...
        # base64 encoding
        msg = NQRustBackupBase64().string_to_base64(bytearray(hmac_md5.digest()))
    ...
    def __check_socket_connection(self):
        ...
        if self.socket is None:
            ...
            if self.auth_credentials_valid:
                ...
                raise nqrustbackup.exceptions.ConnectionLostError(
