...
git_remote="$("$git" remote -v | \
  awk '/git@github.com:\/?nqrustbackup\/nqrustbackup.git \(push\)/ { print $1 }')"
...
if ! grep --quiet --fixed-strings "$git_branch" <<< "nqrustbackup-$version"; then
...
print_block << EOT
You are preparing a release of NQRustBackup. As soon as you push the results of
...
This script will do the following:
  ...
  1b. generate and add cmake/NQRustBackupVersion.cmake
  ...
  2a. remove cmake/NQRustBackupVersion.cmake
...
# the NQRustBackupVersion.cmake file
"$git" commit \
  --quiet \
  --allow-empty \
  --date="$release_ts" \
...
"$git" add \
  ...
  --force \
  -- \
  ./cmake/NQRustBackupVersion.cmake \
  ./CHANGELOG.md
...
"$git" commit \
  --quiet \
  --date="$release_ts" \
  -m "Remove cmake/NQRustBackupVersion.cmake after release"
