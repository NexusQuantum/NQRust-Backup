...
#
#   Copyright (C) 2020-2020 NQRustBackup GmbH & Co. KG
#
...
# * issue numbers   -> link to bugs.nqrustbackup.org
...
for ref_tok in "${ref_tokens[@]}"; do
  case "$ref_tok" in
    Unreleased)
      echo '[unreleased]: https://github.com/nqrustbackup/nqrustbackup/tree/master' \
        >>"$tmp"
      ;;
    [0-9].[0-9].[0-9]|\
    [0-9].[0-9].[0-9][0-9]|\
    ...
    [0-9][0-9].[0-9][0-9].[0-9][0-9])
      printf \
        "[%s]: https://github.com/nqrustbackup/nqrustbackup/releases/tag/Release%%2F%s\n" \
        "$ref_tok" \
        "$ref_tok" \
        >>"$tmp"
      ;;
    ...
    \#[0-9]*)
      ...
      printf "[%s]: https://bugs.nqrustbackup.org/view.php?id=%d\n" \
        "$ref_tok" \
        "$ticket" \
        >>"$tmp"
      ;;
    PR\ \#[0-9]*)
      ...
      printf "[%s]: https://github.com/nqrustbackup/nqrustbackup/pull/%d\n" \
        "$ref_tok" \
        "$pr" \
        >>"$tmp"
      ;;
