# NQRustBackup Developer Tools

This directory contains tools for working with the NQRustBackup source code. As a user, you will likely never need them.
...
4. Test the installation by running `nqrustbackup-check-sources` from outside the `devtools/` directory.
...
#### NQRustBackup Check Sources
Ensure that:
- Pip-Tools are properly installed.
- You are inside a Git repository.

Run `nqrustbackup-check-sources` to check your uncommitted files for the following requirements:
- `clang-format`
- `cmake-format`
- Python `black`
- Trailing spaces
...
- `nqrustbackup-check-sources --since-merge` succeeds.
...
```
This updates the `LICENSE.txt` file at the root of the NQRustBackup repository.
...
Steps:
1. Checkout the branch you want to backport to (e.g., `nqrustbackup-24`).
2. Run:
   ```shell
...
Ensure that:
- You are at the root of the NQRustBackup repository.
...
Options:
- `--ulc` – Package to `nqrustbackup-universal-client` instead of `nqrustbackup`.
