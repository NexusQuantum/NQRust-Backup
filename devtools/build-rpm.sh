...
#
#   Copyright (C) 2024-2024 NQRustBackup GmbH & Co. KG
#
...
while [ "${1:-}" ]; do
  case "$1" in
    --ulc)
      pkg=nqrustbackup-universal-client
      ;;
    --tarball)
      use_tarball=1
      tarball_opts=()
...
if [ -t ${NQRUSTBACKUP_VERSION:+x} ]; then
  NQRUSTBACKUP_VERSION="$(cmake -P get_version.cmake | sed -e 's,^-- ,,')"
fi
...
if rpm -q nqrustbackup-vmware-vix-disklib-devel; then
  rpmbuild_opts+=("--define=vmware 1")
fi

rpmdev-bumpspec \
  --comment="- See https://docs.nqrustbackup.org/release-notes/" \
  --userstring="NQRustBackup Jenkins <noreply@nqrustbackup.com>" \
  --new="${NQRUSTBACKUP_VERSION}-${BUILD_ID:-1}%{?dist}" \
  "${spec}"
