...
at_exit() {
  if [ "$cleanup_version_files" -gt 0 ]; then
    rm -f cmake/NQRustBackupVersion.cmake
  fi
}
trap at_exit EXIT
...
# get version of NQRustBackup to name tarball and directory
...
if [ ! -r cmake/NQRustBackupVersion.cmake ]; then
  cleanup_version_files=1
  cmake -P write_version_files.cmake >/dev/null
fi

(echo -ne 'cmake/NQRustBackupVersion.cmake\0'; "$git" ls-files -z) | \
"$sort" -u -z | \
add_prefix | \
