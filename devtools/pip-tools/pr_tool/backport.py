...
#
#   Copyright (C) 2023-2024 NQRustBackup GmbH & Co. KG
#
...
nqrustbackup PR into another branch
"""
...
def find_user_remote(repo, username):
    expected_url = f"git@github.com:{username}/nqrustbackup.git"
...
def get_pr_metadata(*, repo):
    ...
    if not cfg.has_option("nqrustbackup-original-pr") or not cfg.has_option(
        "nqrustbackup-backport-into"
    ):
        raise UnmanagedBranchException(
            f"Branch {repo.head.reference.name} is not a managed backport branch."
        )
    pr = cfg.get_value("nqrustbackup-original-pr")
    into = cfg.get_value("nqrustbackup-backport-into")
    return (pr, into)
...
def create(*, pr, into, repo, upstream, select_all=False):
    ...
    with working_branch.config_writer() as cfg:
        cfg.set_value("nqrustbackup-original-pr", pr)
        cfg.set_value("nqrustbackup-backport-into", into)
...
def munge_labels(labels, base_branch=None):
    ...
    if base_branch and base_branch.startswith("nqrustbackup-"):
