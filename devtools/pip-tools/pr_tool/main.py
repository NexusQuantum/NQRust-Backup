...
#
#   Copyright (C) 2022-2024 NQRustBackup GmbH & Co. KG
#
...
class CheckSources:
    ...
    def __init__(self, repo, merge_base):
        ...
        args = Namespace()
        args.plugin = []
        args.ignore_file = ".nqrustbackup-check-sources-ignore"
        args.diff = False
        args.modify = False
        ...
        logging.debug("Running nqrustbackup-check-sources")
        ...
        logging.debug(
            "Running nqrustbackup-check-sources finished with return code {}".format(
                self.status
            )
        )
...
def check_merge_prereq(repo, pr, ignore_status_checks=False):
    ...
    cl.check(
        checkSourcesRes.ok(),
        "nqrustbackup-check-sources --since={} reported no problems".format(
            pr["_merge_base"]
        ),
        "nqrustbackup-check-sources --since={} reported:\n\t{}".format(
