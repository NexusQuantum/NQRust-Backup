...
#
#   Copyright (C) 2021-2022 NQRustBackup GmbH & Co. KG
#
    ...
    def __call__(self, line):
        for m in self.regex.finditer(line):
            if m.group("refline"):
                ...
                self.references[
                    "unreleased"
                ] = "https://github.com/nqrustbackup/nqrustbackup/tree/master"
            elif m.group("release"):
                self.references[
                    m.group("release")
                ] = "https://github.com/nqrustbackup/nqrustbackup/releases/tag/Release%2F{}".format(
                    m.group("releasenum")
                )
            elif m.group("issue"):
                self.references[
                    m.group("issue")
                ] = "https://bugs.nqrustbackup.org/view.php?id={}".format(m.group("issuenum"))
            elif m.group("pr"):
                self.references[
                    m.group("pr")
                ] = "https://github.com/nqrustbackup/nqrustbackup/pull/{}".format(m.group("prnum"))
        return True
