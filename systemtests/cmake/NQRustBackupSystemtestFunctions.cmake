...
#
#   Copyright (C) 2021-2026 NQRustBackup GmbH & Co. KG
#
...
macro(create_systemtests_directory)
  ...
  configurefilestosystemtest("systemtests" "scripts" "start_nqrustbackup.sh" @ONLY "")
  ...
  configurefilestosystemtest(
    "systemtests" "scripts" "nqrustbackup-ctl-funcs" @ONLY ""
  )
  configurefilestosystemtest("systemtests" "scripts" "nqrustbackup*" @ONLY "")
  ...
  configurefilestosystemtest("core/src" "scripts" "*_nqrustbackup_*" @ONLY "cats")

  # install special windows start scripts
  if(${CMAKE_SYSTEM_NAME} MATCHES "Windows")
    file(REMOVE ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-fd
         ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-sd
         ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-dir
    )
    file(RENAME ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-fd-win
         ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-fd
    )
    file(RENAME ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-sd-win
         ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-sd
    )
    file(RENAME ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-dir-win
         ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-dir
    )
  else()
    file(REMOVE ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-fd-win
         ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-sd-win
         ${CMAKE_BINARY_DIR}/systemtests/scripts/nqrustbackup-ctl-dir-win
    )
  endif()
...
# create a variable BINARY_NAME_TO_TEST for each binary name nqrustbackup-dir ->
# NQRUSTBACKUP_DIR_TO_TEST nqrustbackup_console -> NQRUSTBACKUP_CONSOLE_TO_TEST
...
macro(find_systemtests_binary_paths SYSTEMTESTS_BINARIES)
  if(RUN_SYSTEMTESTS_ON_INSTALLED_FILES)
    ...
    find_program(
      PYTHON_PLUGIN_TO_TEST python-fd.so PATHS /usr/lib/nqrustbackup/plugins
                                               /usr/lib64/nqrustbackup/plugins
    )
    find_program(
      PYTHON_PLUGIN_TO_TEST python3-fd.so PATHS /usr/lib/nqrustbackup/plugins
                                                /usr/lib64/nqrustbackup/plugins
    )
    find_program(
      CREATE_NQRUSTBACKUP_DATABASE_TO_TEST create_nqrustbackup_database
      PATHS /usr/lib/nqrustbackup/scripts
    )
    find_program(
      PYTHON_PLUGINS_DIR_TO_TEST NQRustBackupFdWrapper.py
      PATHS /usr/lib/nqrustbackup/plugins /usr/lib64/nqrustbackup/plugins
    )
    ...
    get_filename_component(
      SCRIPTS_DIR_TO_TEST ${CREATE_NQRUSTBACKUP_DATABASE_TO_TEST} DIRECTORY
    )
    ...
    set(WEBUI_PUBLIC_DIR_TO_TEST /usr/share/nqrustbackup-webui/public)
    ...
    get_target_output_dir(SD_PLUGINS_DIR_TO_TEST autoxflate-sd)
    if(TARGET nqrustbackupsd-droplet)
      get_target_output_dir(SD_BACKEND_DIR_TO_TEST nqrustbackupsd-droplet)
    endif()
    if(TARGET nqrustbackupsd-gfapi)
      get_target_output_dir(SD_BACKEND_DIR_TO_TEST nqrustbackupsd-gfapi)
    endif()
    if(TARGET nqrustbackupsd-tape)
      get_target_output_dir(SD_BACKEND_DIR_TO_TEST nqrustbackupsd-tape)
    endif()
...
macro(check_for_pamtest)
  ...
  find_package(PAM)
  nqrustbackupfindlibrary("pam_wrapper")
  find_program(PAMTESTER pamtester)

  set(ENV{PAM_WRAPPER_LIBRARIES} "${PAM_WRAPPER_LIBRARIES}")
  execute_process(
    COMMAND
      "${CMAKE_SOURCE_DIR}/systemtests/tests/nqrustbackup_console-pam/bin/check_pam_exec_available.sh"
    WORKING_DIRECTORY "${CMAKE_SOURCE_DIR}/systemtests/tests/nqrustbackup_console-pam/"
  ...
  if(PAM_WRAPPER_LIBRARIES
    ...
    set(ENABLE_NQRUSTBACKUP_CONSOLE_PAM_TEST TRUE)
    message(STATUS "OK: all requirements for pam tests were met.")
  else()
    set(ENABLE_NQRUSTBACKUP_CONSOLE_PAM_TEST FALSE)
    message(
...
if(${CMAKE_SYSTEM_NAME} MATCHES "Windows")
  macro(link_binaries_to_test_to_current_sbin_dir_with_individual_filename)
    ...
    foreach(binary_name ${ALL_BINARIES_BEING_USED_BY_SYSTEMTESTS})
      ...
      string(CONCAT nqrustbackup_XXX_binary ${binary_name_upcase} "_BINARY")
      ...
      if(NOT ${${binary_name_to_test_upcase}} STREQUAL "")
        ...
        foreach(
          ...
          get_filename_component(dst_binary_path "${${nqrustbackup_XXX_binary}}" PATH)
          get_filename_component(dst_file_name "${${nqrustbackup_XXX_binary}}" NAME)
    ...
    if(TARGET nqrustbackup_vadp_dumper)
      if(RUN_SYSTEMTESTS_ON_INSTALLED_FILES)
        create_symlink(
          "/usr/sbin/nqrustbackup_vadp_dumper_wrapper.sh"
          "${CURRENT_SBIN_DIR}/nqrustbackup_vadp_dumper_wrapper.sh"
        )
      else()
        create_symlink(
          "${CMAKE_SOURCE_DIR}/core/src/vmware/vadp_dumper/nqrustbackup_vadp_dumper_wrapper.sh"
          "${CURRENT_SBIN_DIR}/nqrustbackup_vadp_dumper_wrapper.sh"
        )

      endif()
      create_symlink(
        "${CURRENT_SBIN_DIR}/nqrustbackup_vadp_dumper-${TEST_NAME}"
        "${CURRENT_SBIN_DIR}/nqrustbackup_vadp_dumper"
      )
    endif()
  endmacro()
else()
  macro(link_binaries_to_test_to_current_sbin_dir_with_individual_filename)
    foreach(binary_name ${ALL_BINARIES_BEING_USED_BY_SYSTEMTESTS})
      ...
      string(TOUPPER ${binary_name} binary_name_upcase)
      string(CONCAT nqrustbackup_XXX_binary ${binary_name_upcase} "_BINARY")
      # message (STATUS "${nqrustbackup_XXX_binary}")
      set(${nqrustbackup_XXX_binary} ${CURRENT_SBIN_DIR}/${binary_name}-${TEST_NAME})
      # message( "creating symlink ${${nqrustbackup_XXX_binary}}  ->
      # ${${binary_name_to_test_upcase}}" )
      file(CREATE_LINK ${${binary_name_to_test_upcase}} ${${nqrustbackup_XXX_binary}}
           SYMBOLIC
      )
    endforeach()
    ...
    if(TARGET nqrustbackup_vadp_dumper)
      if(RUN_SYSTEMTESTS_ON_INSTALLED_FILES)
        file(CREATE_LINK "/usr/sbin/nqrustbackup_vadp_dumper_wrapper.sh"
             "${CURRENT_SBIN_DIR}/nqrustbackup_vadp_dumper_wrapper.sh" SYMBOLIC
        )
      else()
        file(
          CREATE_LINK
      ...
      file(CREATE_LINK "${CURRENT_SBIN_DIR}/nqrustbackup_vadp_dumper-${TEST_NAME}"
           "${CURRENT_SBIN_DIR}/nqrustbackup_vadp_dumper" SYMBOLIC
      )
    endif()
  endmacro()
endif()
...
macro(prepare_testdir_for_daemon_run)
  ...
  set(confdir ${current_test_directory}/etc/nqrustbackup)
  set(config_directory_dir_additional_test_config
      ${current_test_directory}/etc/nqrustbackup/nqrustbackup-dir.d/additional_test_config
  )
  ...
  set(NQRUSTBACKUP_WEBUI_PUBLIC_DIR ${WEBUI_PUBLIC_DIR_TO_TEST})
  ...
  # create a bin/nqrustbackup and bin/nqrustbackup_console script in every testdir for start/stop
  # and nqrustbackup_console
  file(MAKE_DIRECTORY "${current_test_directory}/bin")
  file(CREATE_LINK "${PROJECT_SOURCE_DIR}/bin/nqrustbackup_console"
       "${current_test_directory}/bin/nqrustbackup_console" SYMBOLIC
  )
  file(CREATE_LINK "${PROJECT_SOURCE_DIR}/bin/nqrustbackup"
       "${current_test_directory}/bin/nqrustbackup" SYMBOLIC
  )
...
macro(prepare_test_python)
  ...
  if(is_grpc)
    ...
    set(dir_python_module_name "grpc:nqrustbackup-grpc-fd-plugin-bridge:python3")
  elseif(is_auto_grpc)
    set(python_module_name "grpc")
    ...
    if(${CMAKE_SYSTEM_NAME} MATCHES "Windows")
      string(
        CONCAT
          pythonpath
          "\""
          "${CMAKE_SOURCE_DIR}/python-nqrustbackup;"
...
macro(prepare_test test_name test_srcdir test_dir)
  ...
  # skip for tests without etc/nqrustbackup ("catalog" test)
  if(EXISTS ${current_test_source_directory}/etc/nqrustbackup)
    file(CREATE_LINK ${CMAKE_SOURCE_DIR}/core/scripts/mtx-changer.conf
         ${current_test_directory}/etc/nqrustbackup/mtx-changer.conf SYMBOLIC
    )
  endif()
...
function(add_systemtest_from_directory test_dir test_basename)
  ...
  if(NOT EXISTS "${test_dir}/test-setup")
    file(CREATE_LINK "${PROJECT_BINARY_DIR}/scripts/start_nqrustbackup.sh"
