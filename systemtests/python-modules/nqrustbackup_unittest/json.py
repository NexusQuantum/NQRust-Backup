...
from nqrustbackup_unittest.base import Base as PythonNQRustBackupBase


class Json(PythonNQRustBackupBase):
    ...
    def check_console(director, name):
        return PythonNQRustBackupJsonBase.check_resource(director, "consoles", name)

    @staticmethod
    def check_jobname(director, name):
        return PythonNQRustBackupJsonBase.check_resource(director, "jobs", name)
    ...
    def _test_no_volume_in_pool(self, console, password, pool):
        ...
        console_poolbotfull = nqrustbackup.bsock.DirectorConsoleJson(
