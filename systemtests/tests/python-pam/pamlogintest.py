...
class PamLoginTest(nqrustbackup_unittest.Base):
    """
    Requires NQRustBackup Console Protocol >= 18.2.
    """
    ...
    def test_pam_login_notls(self):
        ...
        #
        nqrustbackup_password = nqrustbackup.bsock.Password(self.console_pam_password)
    ...
    @unittest.skipUnless(
        nqrustbackup.bsock.DirectorConsole.is_tls_psk_available(), "TLS-PSK is not available."
    )
    ...
    def test_login_with_director_requires_pam_but_protocol_124(self):
        """
        When the Director tries to verify the PAM credentials,
        but the console connects via protocol nqrustbackup_12.4,
        ...
        nqrustbackup_password = nqrustbackup.bsock.Password(self.console_pam_password)
        with self.assertRaises(nqrustbackup.exceptions.ConnectionLostError):
            director = nqrustbackup.bsock.DirectorConsole(
                address=self.director_address,
                port=self.director_port,
                protocolversion=ProtocolVersions.nqrustbackup_12_4,
                name=self.console_pam_username,
                password=nqrustbackup_password,
                **self.director_extra_options
            )
