...
#
#   Copyright (C) 2019-2020 NQRustBackup GmbH & Co. KG
#
...
# * pamtester    to test PAM without the need to run NQRustBackup
# * pam_exec.so  is defined in the nqrustbackup PAM service file.
...
export PAM_WRAPPER_SERVICE_DIR=etc/pam.d/nqrustbackup_discover_pam_exec
...
USERNAME="user"
PASSWORD="user"
echo "$PASSWORD" | LD_PRELOAD=${PAM_WRAPPER_LIBRARIES} pamtester nqrustbackup_discover_pam_exec "$USERNAME" authenticate > /dev/null 2>&1
