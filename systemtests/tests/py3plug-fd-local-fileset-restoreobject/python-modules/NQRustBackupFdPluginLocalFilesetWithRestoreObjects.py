...
#
# Copyright (C) 2014-2025 NQRustBackup GmbH & Co. KG
#
...
#
# NQRustBackup python plugins class that adds files from a local list to
# the backup fileset
...
class NQRustBackupFdPluginLocalFilesetWithRestoreObjects(
    NQRustBackupFdPluginBaseclass.NQRustBackupFdPluginBaseclass
):
    """
    This NQRustBackup-FD-Plugin-Class was created for automated test purposes only.
    It is based on the NQRustBackupFdPluginLocalFileset class that parses a file
    ...
    def __init__(self, plugindef):
        ...
        super(NQRustBackupFdPluginLocalFilesetWithRestoreObjects, self).__init__(
            plugindef, ["filename"]
        )
        ...
        # nqrustbackupfd.SetValue(bVarSinceTime,999)
        # triple_nine = nqrustbackupfd.GetValue(bVarSinceTime)
        # if triple_nine != 999:
    ...
    def start_backup_file(self, savepkt):
        ...
        """
        nqrustbackupfd.DebugMessage(100, "start_backup_file() called\n")
        if not self.files_to_backup:
            nqrustbackupfd.DebugMessage(100, "No files to backup\n")
            return bRC_Stop

        file_to_backup = self.files_to_backup.pop()

        statp = nqrustbackupfd.StatPacket()
        savepkt.statp = statp
    ...
    def end_backup_file(self):
        ...
        """
        nqrustbackupfd.DebugMessage(100, "end_backup_file() entry point in Python called\n")
    ...
    def end_restore_file(self):
        nqrustbackupfd.DebugMessage(100, "end_restore_file() self.FNAME: %s\n" % self.FNAME)
        return bRC_OK
    ...
    def restore_object_data(self, ROP):
        ...
        nqrustbackupfd.DebugMessage(
            100,
            "NQRustBackupFdPluginLocalFilesetWithRestoreObjects:restore_object_data() called with ROP:%s\n"
            % (ROP),
        )
        nqrustbackupfd.DebugMessage(
            100,
