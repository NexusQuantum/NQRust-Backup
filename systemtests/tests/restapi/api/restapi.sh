...
#
# Copyright (C) 2021-2023 NQRustBackup GmbH & Co. KG
#
...
start()
{
    printf "Starting nqrustbackup-restapi: "
    ...
    $PYTHON_EXECUTABLE -m uvicorn nqrustbackup_restapi:app --port ${REST_API_PORT} > ../log/nqrustbackup-restapi.log 2>&1 &
    PID=$!
...
stop()
{
  printf "Stopping nqrustbackup-restapi: "
  if [ -e api.pid ]; then
    PID=$(cat api.pid)
    kill $PID
    rm api.pid
